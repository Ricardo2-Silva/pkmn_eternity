...
# Embedded file name: twisted\logger\_format.py
"""
...
def _formatTraceback(failure):
    ...
    try:
        ...
        traceback = "(UNABLE TO OBTAIN TRACEBACK FROM EVENT):" + unicode(e)
...
def _formatSystem(event):
    ...
    if system is None:
        ...
        system = "{namespace}#{level}".format(namespace=(event.get("log_namespace", "-")),
          level=levelName)
    else:
        try:
    ...
    if includeTraceback:
        ...
        if includeSystem:
            system = "".join([
             ...
             "]",
             " "])
        return "{timeStamp}{system}{eventText}".format(timeStamp=timeStamp,
          system=system,
          eventText=eventText)
