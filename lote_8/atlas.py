...
# Embedded file name: pyglet\image\atlas.py
...
class AllocatorException(Exception):
    __doc__ = "The allocator does not have sufficient free space for the requested\n    image size."
    return
...
class _Strip:
    ...
    def add(self, width, height):
        ...
        elif not height <= self.max_height:
            raise AssertionError
        x, y = self.x, self.y
        self.x += width
...
class Allocator:
    ...
    def alloc(self, width, height):
        ...
        for strip in self.strips:
            if self.width - strip.x >= width:
                if strip.max_height >= height:
                    self.used_area += width * height
                    return strip.add(width, height)

        if self.width >= width:
            if self.height - strip.y2 >= height:
                self.used_area += width * height
                strip.compact()
...
class TextureAtlas:
    __doc__ = "Collection of images within a texture."
