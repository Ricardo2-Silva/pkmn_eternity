...
# Embedded file name: pyglet\media\codecs\ffmpeg_lib\libavformat.py
...
class AVStreamInfo(Structure):
    _fields_ = [
     ...
     (
      "duration_error", POINTER(c_double * 2 * 399)),
     (
...
AVFormatContext._fields_ = [
 ...
 (
  "io_open",
  CFUNCTYPE(c_int, POINTER(AVFormatContext), POINTER(POINTER(AVIOContext)), c_char_p, c_int, POINTER(POINTER(AVDictionary)))),
 (
  "io_close",
  CFUNCTYPE(None, POINTER(AVFormatContext), POINTER(AVIOContext))),
 (
