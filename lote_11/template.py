...
# Embedded file name: twisted\web\template.py
"""
    ...
    def startElementNS(self, namespaceAndName, qname, attrs):
        ...
        if ns == TEMPLATE_NAMESPACE:
            if name == "transparent":
                ...
                el = slot((attrs[(None, 'name')]),
                  default=default, filename=filename,
        ...
        if ns == TEMPLATE_NAMESPACE:
            if name == "attr":
                ...
                el = Tag("", render=render, filename=filename, lineNumber=lineNumber,
                  columnNumber=columnNumber)
        ...
        el = Tag(name,
          attributes=(OrderedDict(nonTemplateAttrs)), render=render, filename=filename,
          lineNumber=lineNumber,
          columnNumber=columnNumber)
        self.stack.append(el)
        self.current.append(el)
...
class XMLFile(object):
    ...
    def __init__(self, path):
        ...
        if not isinstance(path, FilePath):
            warnings.warn("Passing filenames or file objects to XMLFile is deprecated since Twisted 12.1.  Pass a FilePath instead.",
