...
import collections, contextlib, itertools, warnings
from contextlib import contextmanager
import six
from six.moves import range
from cryptography import utils, x509
from cryptography.exceptions import UnsupportedAlgorithm, _Reasons
from cryptography.hazmat._der import INTEGER, NULL, SEQUENCE, encode_der, encode_der_integer
from cryptography.hazmat.backends.interfaces import CMACBackend, CipherBackend, DERSerializationBackend, DHBackend, DSABackend, EllipticCurveBackend, HMACBackend, HashBackend, PBKDF2HMACBackend, PEMSerializationBackend, RSABackend, ScryptBackend, X509Backend
from cryptography.hazmat.backends.openssl import aead
from cryptography.hazmat.backends.openssl.ciphers import _CipherContext
from cryptography.hazmat.backends.openssl.cmac import _CMACContext
from cryptography.hazmat.backends.openssl.decode_asn1 import _CRL_ENTRY_REASON_ENUM_TO_CODE, _CRL_EXTENSION_HANDLERS, _EXTENSION_HANDLERS_BASE, _EXTENSION_HANDLERS_SCT, _OCSP_BASICRESP_EXTENSION_HANDLERS, _OCSP_REQ_EXTENSION_HANDLERS, _OCSP_SINGLERESP_EXTENSION_HANDLERS_SCT, _REVOKED_EXTENSION_HANDLERS, _X509ExtensionParser
...
from cryptography.hazmat.backends.openssl.x509 import _Certificate, _CertificateRevocationList, _CertificateSigningRequest, _RevokedCertificate
...
from cryptography.hazmat.primitives.ciphers.algorithms import AES, ARC4, Blowfish, CAST5, Camellia, ChaCha20, IDEA, SEED, TripleDES
...
@utils.register_interface_if(binding.Binding().lib.Cryptography_HAS_SCRYPT, ScryptBackend)
class Backend(object):
    ...
    def _register_x509_ext_parsers(self):
        ...
        self._ocsp_basicresp_ext_parser = _X509ExtensionParser(self,
        ...
        self._ocsp_singleresp_ext_parser = _X509ExtensionParser(self,
    ...
    def _int_to_bn(self, num, bn=None):
        ...
        if not bn is None:
            ...
            if bn is None:
                bn = self._ffi.NULL
            binary = six.PY2 or num.to_bytes(int(num.bit_length() / 8.0 + 1), "big")
    ...
    def _x509_check_signature_params(self, private_key, algorithm):
        ...
        elif not isinstance(private_key, (
    ...
    def _create_x509_extension(self, handlers, extension):
        if isinstance(extension.value, x509.UnrecognizedExtension):
            ...
            if isinstance(extension.value, x509.TLSFeature):
                asn1 = encode_der(SEQUENCE, *[encode_der(INTEGER, encode_der_integer(x.value)) for x in extension.value])
                value = _encode_asn1_str_gc(self, asn1)
                return self._create_raw_x509_extension(extension, value)
            else:
                if isinstance(extension.value, x509.PrecertPoison):
    ...
    def load_pem_x509_certificate(self, data):
        mem_bio = self._bytes_to_bio(data)
        x509 = self._lib.PEM_read_bio_X509(mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL)
        if x509 == self._ffi.NULL:
            self._consume_errors()
            raise ValueError("Unable to load certificate. See https://cryptography.io/en/latest/faq/#why-can-t-i-import-my-pem-file for more details.")
    ...
    def load_pem_x509_crl(self, data):
        mem_bio = self._bytes_to_bio(data)
        x509_crl = self._lib.PEM_read_bio_X509_CRL(mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL)
        if x509_crl == self._ffi.NULL:
            self._consume_errors()
            raise ValueError("Unable to load CRL. See https://cryptography.io/en/latest/faq/#why-can-t-i-import-my-pem-file for more details.")
        x509_crl = self._ffi.gc(x509_crl, self._lib.X509_CRL_free)
        return _CertificateRevocationList(self, x509_crl)
    ...
    def load_pem_x509_csr(self, data):
        mem_bio = self._bytes_to_bio(data)
        x509_req = self._lib.PEM_read_bio_X509_REQ(mem_bio.bio, self._ffi.NULL, self._ffi.NULL, self._ffi.NULL)
        if x509_req == self._ffi.NULL:
            self._consume_errors()
            raise ValueError("Unable to load request. See https://cryptography.io/en/latest/faq/#why-can-t-i-import-my-pem-file for more details.")
        x509_req = self._ffi.gc(x509_req, self._lib.X509_REQ_free)
        return _CertificateSigningRequest(self, x509_req)
    ...
    def _load_keyParse error at or near `POP_JUMP_IF_TRUE' instruction at offset 272_274
    ...
    def _handle_key_loading_error(self):
        ...
        elif errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.EVP_R_UNKNOWN_PBE_ALGORITHM) or errors[0]._lib_reason_match(self._lib.ERR_LIB_PEM, self._lib.PEM_R_UNSUPPORTED_ENCRYPTION):
            raise UnsupportedAlgorithm("PEM data is encrypted with an unsupported cipher", _Reasons.UNSUPPORTED_CIPHER)
    ...
    def _create_ocsp_basic_response(self, builder, private_key, algorithm):
        ...
        certid = self._lib.OCSP_cert_to_id(evp_md, builder._response._cert._x509, builder._response._issuer._x509)
        self.openssl_assert(certid != self._ffi.NULL)
        ...
        if builder._response._revocation_time is None:
            ...
            rev_time = self._create_asn1_time(builder._response._revocation_time)
        next_update = self._ffi.NULL
        if builder._response._next_update is not None:
            next_update = self._create_asn1_time(builder._response._next_update)
        ...
        self._create_x509_extensions(extensions=(builder._extensions),
          ...
          gc=True)
        res = self._lib.OCSP_basic_sign(basic, responder_cert._x509, private_key._evp_pkey, evp_md, self._ffi.NULL, flags)
        if res != 1:
            errors = self._consume_errors_with_text()
    ...
    def _private_key_bytes(self, encoding, format, encryption_algorithm, key, evp_pkey, cdata):
        if not isinstance(encoding, serialization.Encoding):
            ...
            if not isinstance(format, serialization.PrivateFormat):
                ...
                elif isinstance(encryption_algorithm, serialization.NoEncryption):
                    password = b''
    ...
    def serialize_key_and_certificates_to_pkcs12(self, name, key, cert, cas, encryption_algorithm):
        ...
        with self._zeroed_null_terminated_buf(password) as password_buf:
            with self._zeroed_null_terminated_buf(name) as name_buf:
                p12 = self._lib.PKCS12_create(password_buf, name_buf, key._evp_pkey if key else self._ffi.NULL, cert._x509 if cert else self._ffi.NULL, sk_x509, nid_key, nid_cert, pkcs12_iter, mac_iter, 0)
