...
class SubprocessStreamProtocol(streams.FlowControlMixin, protocols.SubprocessProtocol):
    ...
    def connection_made(self, transport):
        ...
        if stdin_transport is not None:
            self.stdin = streams.StreamWriter(stdin_transport, protocol=self,
              reader=None,
              loop=(self._loop))
...
class Process:
    ...
    def communicate(self, input=None):
        ...
        if self.stderr is not None:
            ...
            stderr = self._noop()
        stdin, stdout, stderr = yield from tasks.gather(stdin, stdout, stderr, loop=(self._loop))
...
def create_subprocess_shell(cmd, stdin=None, stdout=None, stderr=None, loop=None, limit=streams._DEFAULT_LIMIT, **kwds):
 ...
 protocol_factory,
 cmd, stdin=stdin, stdout=stdout, stderr=stderr, **kwds)
...
def create_subprocess_exec(program, *args, stdin=None, stdout=None, stderr=None, loop=None, limit=streams._DEFAULT_LIMIT, **kwds):
 ...
 program, *args, stdin=stdin, 
     stdout=stdout, stderr=stderr, **kwds)
    return Process(transport, protocol, loop)
    if False:
        yield None
