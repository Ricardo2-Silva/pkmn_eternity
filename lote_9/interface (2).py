...
# Embedded file name: pyglet\media\drivers\pulse\interface.py
import sys, weakref
...
class PulseAudioMainLoop:
    ...
    def unlock(self):
        ...
        if not self._pa_threaded_mainloop is not None:
            raise AssertionError
        elif not self._lock_count > 0:
            raise AssertionError
        self._lock_count -= 1
    ...
    def _context_new(self):
        ...
        context = pa.pa_context_new(self._pa_mainloop, app_name.encode("ASCII"))
        return context
...
class PulseAudioContext(PulseAudioLockable):
    __doc__ = "Basic object for a connection to a PulseAudio server."
    ...
    def create_stream(self, audio_format):
        ...
        if not mainloop is not None:
            raise AssertionError
        elif not self.is_ready:
            raise AssertionError
        sample_spec = self.create_sample_spec(audio_format)
        channel_map = None
        stream = pa.pa_stream_new(self._pa_context, str(id(self)).encode("ASCII"), sample_spec, channel_map)
    ...
    def set_input_volume(self, stream, volume):
        ...
        op.execute(pa.pa_context_set_sink_input_volume(self._pa_context, idx, cvolume, op.pa_callback, None))
        return op

    def _get_cvolume_from_linear(self, stream, volume):
        cvolume = pa.pa_cvolume()
        volume = pa.pa_sw_volume_from_linear(volume)
        pa.pa_cvolume_set(cvolume, stream.audio_format.channels, volume)
        return cvolume

    def _connect_callbacks(self):
        self._state_cb_func = pa.pa_context_notify_cb_t(self._state_callback)
        pa.pa_context_set_state_callback(self._pa_context, self._state_cb_func, None)

    def _disconnect_callbacks(self):
        self._state_cb_func = None
        pa.pa_context_set_state_callback(self._pa_context, pa.pa_context_notify_cb_t(0), None)

    def _state_callback(self, context, userdata):
        self.state = pa.pa_context_get_state(self._pa_context)
        assert _debug("PulseAudioContext: state changed to {}".format(self._state_name[self.state]))
        self.signal()
...
class PulseAudioStream(PulseAudioLockable, pyglet.event.EventDispatcher):
    __doc__ = "PulseAudio audio stream."
    ...
    def write(self, audio_data, length=None, seek_mode=pa.PA_SEEK_RELATIVE):
        ...
        if not context is not None:
            raise AssertionError
        elif not self._pa_stream is not None:
            raise AssertionError
        if not self.is_ready:
            raise AssertionError
        else:
            ...
            context.check(pa.pa_stream_write(self._pa_stream, audio_data.data, length, pa.pa_free_cb_t(0), 0, seek_mode))
    ...
    def update_timing_info(self, callback=None):
        ...
        op.execute(pa.pa_stream_update_timing_info(self._pa_stream, op.pa_callback, None))
        return op
    ...
    def get_timing_info(self):
        ...
        if not context is not None:
            ...
            raise AssertionError
        timing_info = context.check_ptr_not_null(pa.pa_stream_get_timing_info(self._pa_stream))
    ...
    def trigger(self, callback=None):
        ...
        op.execute(pa.pa_stream_trigger(self._pa_stream, op.pa_callback, None))
        return op
    ...
    def prebuf(self, callback=None):
        ...
        if not context is not None:
            ...
            raise AssertionError
        op = PulseAudioOperation(context)
        op.execute(pa.pa_stream_prebuf(self._pa_stream, op.pa_callback, None))
        return op
    ...
    def update_sample_rate(self, sample_rate, callback=None):
        ...
        if not context is not None:
            ...
            raise AssertionError
        op = PulseAudioOperation(context)
        op.execute(pa.pa_stream_update_sample_rate(self._pa_stream, int(sample_rate), op.pa_callback, None))
        return op
    ...
    def _cork(self, pause, callback):
        ...
        if not context is not None:
            ...
            raise AssertionError
        op = PulseAudioOperation(context)
        op.execute(pa.pa_stream_cork(self._pa_stream, 1 if pause else 0, op.pa_callback, None))
        return op
    ...
    def _disconnect_callbacks(self):
        ...
        self._cb_write = None
        self._cb_state = None
        pa.pa_stream_set_underflow_callback(self._pa_stream, pa.pa_stream_notify_cb_t(0), None)
        pa.pa_stream_set_write_callback(self._pa_stream, pa.pa_stream_request_cb_t(0), None)
        pa.pa_stream_set_state_callback(self._pa_stream, pa.pa_stream_notify_cb_t(0), None)
...
class PulseAudioOperation(PulseAudioLockable):
    __doc__ = "Asynchronous PulseAudio operation"
    _state_name = {(pa.PA_OPERATION_RUNNING): "Running", 
     (pa.PA_OPERATION_DONE): "Done", 
     (pa.PA_OPERATION_CANCELLED): "Cancelled"}

    def __init__(self, context, callback=None, pa_operation=None, succes_cb_t=pa.pa_stream_success_cb_t):
