...
class _ProactorBaseWritePipeTransport(_ProactorBasePipeTransport, transports.WriteTransport):
    ...
    def write(self, data):
        if not isinstance(data, (bytes, bytearray, memoryview)):
            msg = "data argument must be a bytes-like object, not '%s'" % type(data).__name__
            raise TypeError(msg)
...
class _ProactorSocketTransport(_ProactorReadPipeTransport, _ProactorBaseWritePipeTransport, transports.Transport):
    ...
    def _set_extra(self, sock):
        ...
        try:
            ...
            if self._loop.get_debug():
                logger.warning("getsockname() failed on %r", sock,
                  exc_info=True)
        ...
        if "peername" not in self._extra:
            try:
                ...
                if self._loop.get_debug():
                    logger.warning("getpeername() failed on %r", sock,
                      exc_info=True)
...
class BaseProactorEventLoop(base_events.BaseEventLoop):
    ...
    def _make_ssl_transport(self, rawsock, protocol, sslcontext, waiter=None, *, server_side=False, server_hostname=None, extra=None, server=None):
        if not sslproto._is_sslproto_available():
            raise NotImplementedError("Proactor event loop requires Python 3.5 or newer (ssl.MemoryBIO) to support SSL")
                ...
                if f is not None:
                    ...
                    if sslcontext is not None:
                        self._make_ssl_transport(conn,
                          protocol, sslcontext, server_side=True, extra={"peername": addr},
                          server=server)
                    else:
                        self._make_socket_transport(conn,
                          protocol, extra={"peername": addr},
                          server=server)
                if self.is_closed():
                    return
