...
def _hash_algorithm(backend, cert_id):
    ...
    res = backend._lib.OCSP_id_get0_info(backend._ffi.NULL, asn1obj, backend._ffi.NULL, backend._ffi.NULL, cert_id)
    backend.openssl_assert(res == 1)
    ...
    try:
        ...
        raise UnsupportedAlgorithm("Signature algorithm OID: {} not recognized".format(oid))
