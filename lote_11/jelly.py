...
# Embedded file name: twisted\spread\jelly.py
"""
...
def _createBlank(cls):
    ...
    if not _PY3:
        if isinstance(cls, _OldStyleClass):
...
class _Jellier:
    ...
    def jellyParse error at or near `JUMP_IF_TRUE_OR_POP' instruction at offset 316_318
...
class _Unjellier:
    ...
    def unjelly(self, obj):
        if type(obj) is not list:
            ...
            if not self.taster.isModuleAllowed(modName):
                raise InsecureJelly("Module %s not allowed (in type %s)." % (modName, jelTypeText))
    ...
    def _unjelly_instance(self, rest):
        ...
        """
        warnings.warn_explicit("Unjelly support for the instance atom is deprecated since Twisted 15.0.0.  Upgrade peer for modern instance support.",
    ...
    def _unjelly_method(self, rest):
        ...
        elif im_name in im_class.__dict__:
            if im_self is None:
