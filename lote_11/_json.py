...
# Embedded file name: twisted\logger\_json.py
"""
...
def failureAsJSON(failure):
    ...
    @rtype: L{dict}
    """
    return dict((failure.__getstate__()),
      type=dict(__module__=(failure.type.__module__),
...
def eventsFromJSONLogFile(inFile, recordSeparator=None, bufferSize=4096):
    ...
    def eventFromBytearray(record):
        try:
            ...
            log.error("Unable to decode UTF-8 for JSON record: {record!r}",
              record=(bytes(record)))
            return
        else:
            try:
                ...
                log.error("Unable to read JSON record: {record!r}",
                  record=(bytes(record)))
                return
    ...
    if recordSeparator == b'':
        ...
        def eventFromRecord(record):
            if record[-1] == ord("\n"):
                ...
                log.error("Unable to read truncated JSON record: {record!r}",
                  record=(bytes(record)))
                return
