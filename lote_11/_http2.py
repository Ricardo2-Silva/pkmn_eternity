...
# Embedded file name: twisted\web\_http2.py
"""
...
import priority, h2.config, h2.connection, h2.errors, h2.events, h2.exceptions
from twisted.internet.defer import Deferred
from twisted.internet.error import ConnectionLost
from twisted.internet.interfaces import IProtocol, ITransport, IConsumer, IPushProducer, ISSLTransport
...
if sys.version_info < (2, 7, 4):
    warnings.warn("HTTP/2 cannot be enabled because this version of Python is too old, and does not fully support memoryview objects.",
      UserWarning,
      stacklevel=2)
...
class H2Connection(Protocol, TimeoutMixin):
    ...
    def __init__(self, reactor=None):
        ...
        self._maxBufferedControlFrameBytes = 17408
    ...
    def _sendPrioritisedData(self, *args):
        ...
        if not self._stillProducing:
            ...
            elif len(frameData) > maxFrameSize:
    ...
    def _requestAborted(self, event):
        ...
        """
        stream = self.streams[event.stream_id]
        stream.connectionLost(ConnectionLost("Stream reset with code %s" % event.error_code))
    ...
    def _handlePriorityUpdate(self, event):
        ...
        """
        try:
            self.priority.reprioritize(stream_id=(event.stream_id),
              depends_on=(event.depends_on or None),
              weight=(event.weight),
              exclusive=(event.exclusive))
        except priority.MissingStreamError:
            self.priority.insert_stream(stream_id=(event.stream_id),
              depends_on=(event.depends_on or None),
              weight=(event.weight),
              exclusive=(event.exclusive))
            self.priority.block(event.stream_id)
    ...
    def _tryToWriteControlData(self):
        ...
        if self._consumerBlocked is None and not self._bufferedControlFrames:
            ...
            if self._bufferedControlFrameBytes >= self._maxBufferedControlFrameBytes:
                self._log.error("Maximum number of control frame bytes buffered: {bufferedControlFrameBytes} > = {maxBufferedControlFrameBytes}. Aborting connection to client: {client} ",
...
class H2Stream(object):
    ...
    def windowUpdated(self):
        ...
        if not self.producer:
            ...
            if self._producerProducing:
                ...
                return remainingWindow > 0 or None
    ...
    def unregisterProducer(self):
        ...
        if self.producer is not None:
            if not self.hasStreamingProducer:
