...
def visiblename(name, all=None, obj=None):
    ...
    if name in frozenset({'__file__', '__credits__', '__author__', '__slots__', '__module__', '__builtins__', '__name__', '__doc__', '__loader__', '__cached__', '__qualname__', '__path__', '__spec__', '__package__', '__date__', '__version__'}):
        return 0
    else:
...
def safeimport(path, forceload=0, cache={}):
    ...
    try:
        ...
        elif issubclass(exc, ImportError):
            if value.name == path:
                return
            raise ErrorDuringImport(path, sys.exc_info())
...
class Doc:
    ...
    def getdocloc(self, object, basedir=os.path.join(sys.base_exec_prefix, "lib", "python%d.%d" % sys.version_info[:2])):
        ...
        if isinstance(object, type(os)):
            if (object.__name__ in ('errno', 'exceptions', 'gc', 'imp', 'marshal',
                                    'posix', 'signal', 'sys', '_thread', 'zipimport') or file.startswith(basedir) and not file.startswith(os.path.join(basedir, "site-packages"))) and object.__name__ not in ('xml.etree',
                                                                                                                                                                                                               'test.pydoc_mod'):
