...
# Embedded file name: twisted\internet\tcp.py
"""
...
import socket, sys, operator, os, struct, attr
    ...
    def _dataReceived(self, data):
        if not data:
            ...
            if rval is not None:
                ...
                warningString = deprecate.getDeprecationWarningString(offender,
                  (versions.Version("Twisted", 11, 0, 0)), format=warningFormat)
    ...
    for _ in accepts:
        try:
            ...
            if e.args[0] in (EWOULDBLOCK, EAGAIN):
                ...
                if e.args[0] == EMFILE:
                    if reservedFD.available():
                        logger.info("EMFILE encountered; releasing reserved file descriptor.")
                        with reservedFD:
                            clientsToClose = _accept(logger, accepts, listener, reservedFD)
                            for clientToClose, closedAddress in clientsToClose:
                                clientToClose.close()
                                logger.info("EMFILE recovery: Closed socket from {address}", address=closedAddress)

                            logger.info("Re-reserving EMFILE recovery file descriptor.")
                        return
                if e.args[0] in _ACCEPT_ERRORS:
                    logger.info("Could not accept new connection ({acceptError})", acceptError=(errorcode[e.args[0]]))
                    return
                raise
        else:
            yield (
    ...
    def doRead(self):
        ...
        try:
            if platformType == "posix":
                ...
                numAccepts = 1
            with _BuffersLogs(self._logger.namespace, self._logger.observer) as bufferingLogger:
                accepted = 0
