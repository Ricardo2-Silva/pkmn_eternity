...
class DocTestFinder:
    ...
    def _findParse error at or near `POP_JUMP_IF_TRUE' instruction at offset 312_314
    ...
    def _get_test(self, obj, name, module, globs, source_lines):
        ...
        if isinstance(obj, str):
            ...
            if self._exclude_empty:
                ...
                if filename[-4:] == ".pyc":
                    filename = filename[:-1]
        return self._parser.get_doctestdocstringglobsnamefilenamelineno
    ...
    def __run(self, test, compileflags, out):
        ...
        for examplenum, example in enumerate(test.examples):
            ...
            if self.optionflags & SKIP:
                ...
                try:
                    exec(compileexample.sourcefilename"single"compileflags1, test.globs)
...
class OutputChecker:
    ...
    def output_difference(self, example, got, optionflags):
        ...
        elif self._do_a_fancy_diff(want, got, optionflags):
...
class DebugRunner(DocTestRunner):
    ...
    def run(self, test, compileflags=None, out=None, clear_globs=True):
        r = DocTestRunner.runselftestcompileflagsoutFalse
        if clear_globs:
            test.globs.clear()
        return r
    ...
    if package:
        if not module_relative:
            ...
            if raise_on_error:
                ...
                runner = DocTestRunner(verbose=verbose, optionflags=optionflags)
            test = parser.get_doctesttextglobsnamefilename0
            runner.run(test)
            if report:
                runner.summarize()
        if master is None:
...
def DocFileTest(path, module_relative=True, package=None, globs=None, parser=DocTestParser(), encoding=None, **options):
    ...
    test = parser.get_doctestdocglobsnamepath0
