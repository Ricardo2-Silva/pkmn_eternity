...
# Embedded file name: twisted\web\server.py
"""
...
class Request(Copyable, http.Request, components.Componentized):
        ...
        if not self.startedWriting:
            ...
            if needsCT:
                if contentType is None:
                    if self.defaultContentType is not None:
                        if not contentLengthZero:
                            self.responseHeaders.setRawHeaders(b'content-type', [self.defaultContentType])
        elif not self._inFakeHead:
            if self._encoder:
    ...
    def render(self, resrc):
            ...
            if self.method == b'HEAD':
                if b'GET' in allowedMethods:
                    ...
                    if body is NOT_DONE_YET:
                        self._log.info("Tried to fake a HEAD request for {resrc}, but it got away from me.",
                          resrc=resrc)
                    else:
        ...
        if body is NOT_DONE_YET:
            ...
            if self.method == b'HEAD':
                if len(body) > 0:
                    self._log.info("Warning: HEAD request {slf} for resource {resrc} is returning a message body. I think I'll eat it.",
                      slf=self,
                      resrc=resrc)
                    self.setHeader(b'content-length', intToBytes(len(body)))
                self.write(b'')
    ...
    def processingFailed(self, reason):
        ...
        if self.site.displayTracebacks:
            body = b'<html><head><title>web.Server Traceback (most recent call last)</title></head><body><b>web.Server Traceback (most recent call last):</b>\n\n' + util.formatFailure(reason) + b'\n\n</body></html>\n'
        else:
            body = b'<html><head><title>Processing Failed</title></head><body><b>Processing Failed</b></body></html>'
