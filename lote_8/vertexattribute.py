...
# Embedded file name: pyglet\graphics\vertexattribute.py
...
def create_attribute(format):
    ...
    if not match:
        raise AssertionError("Invalid attribute format %r" % format)
    else:
...
class AbstractAttribute:
    __doc__ = "Abstract accessor for an attribute in a mapped buffer.\n    "
    _fixed_count = None
...
class ColorAttribute(AbstractAttribute):
    __doc__ = "Color vertex attribute."
    plural = "colors"
...
class FogCoordAttribute(AbstractAttribute):
    __doc__ = "Fog coordinate attribute."
    plural = "fog_coords"
...
class NormalAttribute(AbstractAttribute):
    __doc__ = "Normal vector attribute."
    plural = "normals"
    _fixed_count = 3
...
class SecondaryColorAttribute(AbstractAttribute):
    __doc__ = "Secondary color attribute."
...
class TexCoordAttribute(AbstractAttribute):
    __doc__ = "Texture coordinate attribute."
    plural = "tex_coords"
...
class VertexAttribute(AbstractAttribute):
    __doc__ = "Vertex coordinate attribute."
    plural = "vertices"

    def __init__(self, count, gl_type):
        if not count > 1:
            raise AssertionError("Vertex attribute must have count of 2, 3 or 4")
        elif not gl_type in (GL_SHORT, GL_INT, GL_INT, GL_FLOAT, GL_DOUBLE):
            raise AssertionError("Vertex attribute must have signed type larger than byte")
...
class GenericAttribute(AbstractAttribute):
    __doc__ = "Generic vertex attribute, used by shader programs."
