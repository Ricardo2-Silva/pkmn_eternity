...
# Embedded file name: pyglet\window\xlib\__init__.py
...
class XlibException(WindowException):
    __doc__ = "An X11-specific exception.  This exception is probably a programming\n    error in pyglet."
    return
...
class XlibWindow(BaseWindow):
    ...
    def _create_xdnd_atoms(self, display):
        self._xdnd_atoms = {'XdndAware':xlib.XInternAtom(display, asbytes("XdndAware"), False), 
    ...
    def set_mouse_platform_visible(self, platform_visible=None):
        if not self._window:
            ...
            xlib.XFreePixmap(self._x_display, bitmap)
        elif isinstance(self._mouse_cursor, ImageMouseCursor):
            if self._mouse_cursor.hw_drawable:
    ...
    def _event_text_symbol(self, ev):
        ...
        if text:
            if unicodedata.category(text) == "Cc":
                if text != "\r":
                    text = None
        symbol = symbol.value
    ...
    def _event_key_view(self, ev):
        if not _can_detect_autorepeat and ev.type == xlib.KeyRelease:
            ...
            while 1:
                ...
                if auto_event.type == xlib.KeyRelease:
                    pass
                elif ev.xkey.keycode == auto_event.xkey.keycode:
        ...
        if ev.type == xlib.KeyPress:
            if symbol and (not _can_detect_autorepeat or symbol not in self.pressed_keys):
                ...
                if text and not modifiers_ctrl:
                    self.dispatch_event("on_text", text)
    ...
    def _event_clientmessage(self, ev):
        ...
        elif self._enable_xsync:
            pass
        if atom == xlib.XInternAtom(ev.xclient.display, asbytes("_NET_WM_SYNC_REQUEST"), False):
    ...
    def _event_selection_notification(self, ev):
        if ev.xselection.property != 0 and ev.xselection.selection == self._xdnd_atoms["XdndSelection"]:
            if self._xdnd_format:
                data, count = self.get_single_property(ev.xselection.requestor, ev.xselection.property, ev.xselection.target)
