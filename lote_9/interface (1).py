...
# Embedded file name: pyglet\media\drivers\openal\interface.py
...
class OpenALObject:
    __doc__ = "Base class for OpenAL objects."
    ...
    def _check_error(cls, message=None):
        ...
        if error_code != 0:
            ...
            raise OpenALException(message=message, error_code=error_code,
...
class OpenALDevice(OpenALObject):
    ...
    def get_version(self):
        major = alc.ALCint()
        minor = alc.ALCint()
        alc.alcGetIntegerv(self._al_device, alc.ALC_MAJOR_VERSION, ctypes.sizeof(major), major)
        self.check_context_error("Failed to get version.")
        alc.alcGetIntegerv(self._al_device, alc.ALC_MINOR_VERSION, ctypes.sizeof(minor), minor)
        self.check_context_error("Failed to get version.")
...
class OpenALBuffer(OpenALObject):
    _format_map = {(1, 8): (al.AL_FORMAT_MONO8), 
    ...
    def delete(self):
        if self._al_buffer is not None:
            if self.context():
                if self.is_valid:
    ...
    def data(self, audio_data, audio_format, length=None):
        ...
        al.alBufferData(self._al_buffer, al_format, audio_data.data, length, audio_format.sample_rate)
        self._check_error("Failed to add data to buffer.")


class OpenALBufferPool(OpenALObject):
    __doc__ = "At least Mac OS X doesn't free buffers when a source is deleted; it just\n    detaches them from the source.  So keep our own recycled queue.\n    "
