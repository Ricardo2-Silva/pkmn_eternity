...
# Embedded file name: pyglet\event.py
...
class EventException(Exception):
    __doc__ = "An exception raised when an event handler could not be attached.\n    "
    return


class EventDispatcher:
    __doc__ = "Generic event dispatcher interface.\n\n    See the module docstring for usage.\n    "
    _event_stack = ()
    ...
    def _get_handlers(self, args, kwargs):
        ...
        for obj in args:
            if inspect.isroutine(obj):
                ...
                if inspect.ismethod(obj):
                    yield (
                     name, WeakMethod(obj, partial(self._remove_handler, name)))
                else:
                    yield (
                     name, obj)
            else:
        ...
        for name, handler in kwargs.items():
            ...
            if inspect.ismethod(handler):
                yield (
                 name, WeakMethod(handler, partial(self._remove_handler, name)))
            else:
                yield (
                 name, handler)
    ...
    def dispatch_event(self, event_type, *args):
        ...
        if not hasattr(self, "event_types"):
            raise AssertionError("No events registered on this EventDispatcher. You need to register events with the class method EventDispatcher.register_event_type('event_name').")
        elif not event_type in self.event_types:
            raise AssertionError("%r not found in %r.event_types == %r" % (event_type, self, self.event_types))
        invoked = False
        for frame in list(self._event_stack):
            handler = frame.get(event_type, None)
            if not handler:
                pass
            elif isinstance(handler, WeakMethod):
                handler = handler()
    ...
    def _raise_dispatch_exception(event_type, args, handler, exception):
        ...
        elif n_handler_args > n_args >= n_handler_args - len(handler_defaults):
            if handler_defaults:
