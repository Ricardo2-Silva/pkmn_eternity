...
# Embedded file name: pyglet\media\drivers\xaudio2\interface.py
import weakref
...
class XAudio2Driver:
    ...
    def __init__(self):
        ...
        """
        if not _debug("Constructing XAudio2Driver"):
    ...
    def pooled_voices(self):
        return [voice for voices in self._voice_pool.values() for voice in iter(voices)]
    ...
    def enable_3d(self):
        ...
        pyglet.clock.schedule_interval_soft(self._calculate_3d_sources, 0.06666666666666667)
    ...
    def _calculate3d(self, listener, emitter):
        lib.X3DAudioCalculate(self._x3d_handle, listener, emitter, lib.default_dsp_calculation, self._dsp_settings)
    ...
    def _apply3d(self, voice, commit):
        ...
        """
        voice.SetOutputMatrix(self._master_voice, 1, self._mvoice_details.InputChannels, self._dsp_settings.pMatrixCoefficients, commit)
    ...
    def get_source_voice(self, source, player):
        ...
        voice_key = (
         source.audio_format.channels, source.audio_format.sample_size)
