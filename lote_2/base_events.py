...
def _is_stream_socket(sock_type):
    if hasattr(socket, "SOCK_NONBLOCK"):
        return sock_type & 15 == socket.SOCK_STREAM
    else:
...
def _is_dgram_socket(sock_type):
    if hasattr(socket, "SOCK_NONBLOCK"):
        return sock_type & 15 == socket.SOCK_DGRAM
    else:
...
def _ensure_resolved(address, *, family=0, type=socket.SOCK_STREAM, proto=0, flags=0, loop):
    ...
    if info is not None:
        ...
        return loop.getaddrinfo(host, port, family=family, type=type, proto=proto,
          flags=flags)
...
if hasattr(socket, "TCP_NODELAY"):
    ...
    def _set_nodelay(sock):
        if sock.family in {socket.AF_INET, socket.AF_INET6}:
            if _is_stream_socket(sock.type):
...
class BaseEventLoop(events.AbstractEventLoop):
    ...
    def _make_ssl_transport(self, rawsock, protocol, sslcontext, waiter=None, *, server_side=False, server_hostname=None, extra=None, server=None):
    ...
    def _asyncgen_firstiter_hook(self, agen):
        if self._asyncgens_shutdown_called:
            warnings.warn(("asynchronous generator {!r} was scheduled after loop.shutdown_asyncgens() call".format(agen)),
              ResourceWarning,
              source=self)
    ...
    def shutdown_asyncgens(self):
        ...
        shutdown_coro = (tasks.gather)(*[ag.aclose() for ag in closing_agens], return_exceptions=True, 
         loop=self)
        results = yield from shutdown_coro
        for result, agen in zip(results, closing_agens):
            if isinstance(result, Exception):
                self.call_exception_handler({'message':"an error occurred during closing of asynchronous generator {!r}".format(agen), 
                 'exception':result, 
                 'asyncgen':agen})
    ...
    def create_connection(self, protocol_factory, host=None, port=None, *, ssl=None, family=0, proto=0, flags=0, sock=None, local_addr=None, server_hostname=None):
        ...
        if server_hostname is not None:
            if not ssl:
                ...
                f1 = _ensure_resolved((host, port), family=family, type=(socket.SOCK_STREAM),
                  proto=proto,
                  flags=flags,
                  loop=self)
                fs = [f1]
                if local_addr is not None:
                    f2 = _ensure_resolved(local_addr, family=family, type=(socket.SOCK_STREAM),
                      proto=proto,
                      flags=flags,
                      loop=self)
                    fs.append(f2)
            ...
            if f2 is not None:
                ...
                raise laddr_infos or OSError("getaddrinfo() returned empty list")
            exceptions = []
    ...
    @coroutine
    def create_datagram_endpointParse error at or near `POP_JUMP_IF_TRUE' instruction at offset 248_250

    @coroutine
    def _create_server_getaddrinfo(self, host, port, family, flags):
        infos = yield from _ensure_resolved((host, port), family=family, type=(socket.SOCK_STREAM),
          flags=flags,
          loop=self)
        if not infos:
        ...
        if host is not None or port is not None:
            if sock is not None:
                ...
                if host == "":
                    hosts = [
                     None]
                elif isinstance(host, str) or not isinstance(host, collections.Iterable):
                    hosts = [
                     host]
                else:
                    hosts = host
    ...
    @coroutine
    def subprocess_exec(self, protocol_factory, program, *args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=False, shell=False, bufsize=0, **kwargs):
        ...
        try:
            ...
            if enabled:
                if current_wrapper not in (None, wrapper):
                    warnings.warn("loop.set_debug(True): cannot set debug coroutine wrapper; another wrapper is already set %r" % current_wrapper, RuntimeWarning)
                else:
                    set_wrapper(wrapper)
                    self._coroutine_wrapper_set = True
            elif current_wrapper not in (None, wrapper):
                warnings.warn("loop.set_debug(False): cannot unset debug coroutine wrapper; another wrapper was set %r" % current_wrapper, RuntimeWarning)
            else:
                set_wrapper(None)
                self._coroutine_wrapper_set = False
