...
# Embedded file name: pyglet\resource.py
...
.. versionadded:: 1.1
"""
import io, os, sys, zipfile, weakref, pyglet

class ResourceNotFoundException(Exception):
    __doc__ = "The named resource was not found on the search path."
...
class FileLocation(Location):
    __doc__ = "Location on the filesystem.\n    "
...
class ZIPLocation(Location):
    __doc__ = "Location within a ZIP file.\n    "
...
class Loader:
        ...
        for path in self.path:
            if path.startswith("@"):
                ...
                if not os.path.isabs(path):
                    assert "\\\\" not in path, "Backslashes are not permitted in relative paths"
    ...
    def animation(self, name, flip_x=False, flip_y=False, rotate=0, border=1):
        ...
        try:
            ...
            bin = self._get_texture_atlas_bin(animation.get_max_width(), animation.get_max_height(), border)
            if bin:
