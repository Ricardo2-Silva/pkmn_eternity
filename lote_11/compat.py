...
# Embedded file name: twisted\python\compat.py
"""
...
import inspect, os, platform, socket, struct, sys, tokenize
    ...
    if not addr:
        ...
        if af == socket.AF_INET:
            ...
            if af == getattr(socket, "AF_INET6", "AF_INET6"):
                if "%" in addr:
                    if addr.count("%") > 1 or addr.index("%") == 0:
                        raise ValueError("illegal IP address string passed to inet_pton")
                    else:
                        addr = addr.split("%")[0]
                        parts = addr.split(":")
...
def _get_async_param(isAsync=None, **kwargs):
    ...
    """
    if "async" in kwargs:
        warnings.warn("'async' keyword argument is deprecated, please use isAsync",
          DeprecationWarning,
          stacklevel=2)
    else:
        if isAsync is None:
