...
# Embedded file name: twisted\internet\endpoints.py
"""
...
from twisted.internet.address import _ProcessAddress, HostnameAddress, IPv4Address, IPv6Address
from twisted.internet.interfaces import IStreamServerEndpointStringParser, IStreamClientEndpointStringParserWithReactor, IResolutionReceiver, IReactorPluggableNameResolver, IHostnameResolver
...
try:
    ...
    from twisted.internet.ssl import optionsForClientTLS, PrivateCertificate, Certificate, KeyPair, CertificateOptions, trustRootFromCertificates
...
class StandardIOEndpoint(object):
    ...
    def listen(self, stdioProtocolFactory):
        ...
        """
        return defer.execute((self._stdio), (stdioProtocolFactory.buildProtocol(PipeAddress())),
...
class _WrapIProtocol(ProcessProtocol):
    ...
    def childDataReceived(self, childFD, data):
        ...
        if childFD == 2:
            if self.errFlag == StandardErrorBehavior.LOG:
                log.msg(format="Process %(executable)r wrote stderr unhandled by %(protocol)s: %(data)s",
    ...
    def processEnded(self, reason):
        ...
        """
        if reason.check(error.ProcessDone) == error.ProcessDone:
...
class _TCPServerEndpoint(object):
    ...
    def listen(self, protocolFactory):
        ...
        socket
        """
        return defer.execute((self._reactor.listenTCP), (self._port),
          protocolFactory,
    ...
    def connect(self, protocolFactory):
        ...
        try:
            ...
            self._reactor.connectTCP((self._host),
              (self._port), wf, timeout=(self._timeout),
...
class TCP6ClientEndpoint(object):
    ...
    def _resolvedHostConnect(self, resolvedHost, protocolFactory):
        ...
        try:
            wf = _WrappingFactory(protocolFactory)
            self._reactor.connectTCP(resolvedHost, (self._port), wf, timeout=(self._timeout),
              bindAddress=(self._bindAddress))
            return wf._onConnection
        except:
            return defer.fail()
...
class _SimpleHostnameResolver(object):
    ...
    def resolveHostName(self, resolutionReceiver, hostName, portNumber=0, addressTypes=None, transportSemantics='TCP'):
        ...
        def ebLog(error):
            self._log.failure("while looking up {name} with {callable}", error,
              name=hostName, callable=(self._nameResolution))
...
class HostnameEndpoint(object):
    ...
    def _getNameResolverAndMaybeWarn(self, reactor):
        ...
        if not IReactorPluggableNameResolver.providedBy(reactor):
            warningString = deprecate.getDeprecationWarningString((reactor.__class__),
              (Version("Twisted", 17, 5, 0)),
              format="Passing HostnameEndpoint a reactor that does not provide IReactorPluggableNameResolver (%(fqpn)s) was deprecated in %(version)s",
              replacement="a reactor that provides IReactorPluggableNameResolver")
    ...
    def _hostAsBytesAndText(host):
        ...
        if isinstance(host, bytes):
            if isIPAddress(host) or isIPv6Address(host):
                return (
                 False, host, host.decode("ascii"))
            try:
                return (
                 False, host, _idnaText(host))
            except UnicodeError:
    ...
    def connect(self, protocolFactory):
        ...
        if self._badHostname:
            ...
            self._nameResolver.resolveHostName(EndpointReceiver,
              (self._hostText), portNumber=(self._port))
            d.addErrback((lambda ignored: defer.fail(error.DNSLookupError("Couldn't find the hostname '{}'".format(self._hostStr)))))
            ...
            """
                if not endpoints:
                    raise error.DNSLookupError("no results for hostname lookup: {}".format(self._hostStr))
...
class SSL4ServerEndpoint(object):
    ...
    def listen(self, protocolFactory):
        ...
        TCP socket.
        """
        return defer.execute((self._reactor.listenSSL), (self._port), protocolFactory,
...
class SSL4ClientEndpoint(object):
    ...
    def connect(self, protocolFactory):
        ...
        try:
            wf = _WrappingFactory(protocolFactory)
            self._reactor.connectSSL((self._host),
              (self._port), wf, (self._sslContextFactory), timeout=(self._timeout),
              bindAddress=(self._bindAddress))
            return wf._onConnection
        except:
            return defer.fail()
...
class UNIXServerEndpoint(object):
    ...
    def listen(self, protocolFactory):
        ...
        """
        return defer.execute((self._reactor.listenUNIX), (self._address), protocolFactory,
          backlog=(self._backlog),
          mode=(self._mode),
...
class UNIXClientEndpoint(object):
    ...
    def connect(self, protocolFactory):
        ...
        try:
            wf = _WrappingFactory(protocolFactory)
            self._reactor.connectUNIX((self._path),
              wf, timeout=(self._timeout),
...
def _parseUNIX(factory, address, mode='666', backlog=50, lockfile=True):
    ...
    return (
     (
      address, factory),
     {'mode':int(mode, 8), 
      'backlog':int(backlog),  'wantPID':bool(int(lockfile))})
...
def _parseSSL(factory, port, privateKey='server.pem', certKey=None, sslmethod=None, interface='', backlog=50, extraCertChain=None, dhParameters=None):
    ...
    cf = (ssl.CertificateOptions)(privateKey=privateCertificate.privateKey.original, 
     certificate=privateCertificate.original, 
     extraCertChain=chainCertificates, 
     dhParameters=dhParameters, **kw)
    return (
     (
      int(port), factory, cf),
     {'interface':interface, 
...
def _tokenize(description):
    ...
    backslash = _matchingString("\\", description)
    current = empty
    ops = colon + equals
...
def _parseServer(description, factory):
    ...
    if parser is None:
        plugin = _matchPluginToPrefix(getPlugins(IStreamServerEndpointStringParser), endpointType)
        return (
         plugin, args[1:], kw)
    else:
        return (
...
def _parseClientSSLOptions(kwargs):
    ...
    if hostname is not None:
        configuration = optionsForClientTLS((_idnaText(hostname)),
          trustRoot=trustRoot, clientCertificate=clientCertificate)
    elif clientCertificate is not None:
    ...
    configuration = CertificateOptions(trustRoot=trustRoot,
...
class _WrapperEndpoint(object):
    ...
    def connect(self, protocolFactory):
        ...
        """
        return self._wrappedEndpoint.connect(self._wrapperFactory(protocolFactory)).addCallback((lambda protocol: protocol.wrappedProtocol))
...
def wrapClientTLS(connectionCreator, wrappedEndpoint):
    ...
    if TLSMemoryBIOFactory is None:
        raise NotImplementedError("OpenSSL not available. Try `pip install twisted[tls]`.")
    return _WrapperEndpoint(wrappedEndpoint, (lambda protocolFactory: TLSMemoryBIOFactory(connectionCreator, True, protocolFactory)))
...
def _parseClientTLS(reactor, host, port, timeout=b'30', bindAddress=None, certificate=None, privateKey=None, trustRoots=None, endpoint=None, **kwargs):
    ...
    bindAddress = bindAddress if isinstance(bindAddress, unicode) or bindAddress is None else bindAddress.decode("utf-8")
    port = int(port)
    timeout = int(timeout)
    return wrapClientTLS(optionsForClientTLS(host,
      trustRoot=(_parseTrustRootPath(trustRoots)), clientCertificate=(_privateCertFromPaths(certificate, privateKey))), clientFromString(reactor, endpoint) if endpoint is not None else HostnameEndpoint(reactor, _idnaBytes(host), port, timeout, bindAddress))
