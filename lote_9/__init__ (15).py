...
# Embedded file name: pyglet\window\__init__.py
...
"""
import sys, math, warnings, pyglet, pyglet.window.key, pyglet.window.event
from pyglet import gl
...
class WindowException(Exception):
    __doc__ = "The root exception for all window-related errors."
    return


class NoSuchDisplayException(WindowException):
    __doc__ = "An exception indicating the requested display is not available."
    return


class NoSuchConfigException(WindowException):
    __doc__ = "An exception indicating the requested configuration is not\n    available."
    return


class NoSuchScreenModeException(WindowException):
    __doc__ = "An exception indicating the requested screen resolution could not be\n    met."
    return


class MouseCursorException(WindowException):
    __doc__ = "The root exception for all mouse cursor-related errors."
    return


class MouseCursor:
    __doc__ = "An abstract mouse cursor."
    gl_drawable = True
    hw_drawable = False
...
class Projection2D(Projection):
    ...
    def set(self, window_width, window_height, viewport_width, viewport_height):
        ...
        gl.glLoadIdentity()
        gl.glOrtho(0, max(1, window_width), 0, max(1, window_height), -3000, 3000)
...
class _WindowMetaclass(type):
    __doc__ = "Sets the _platform_event_names class variable on the window\n    subclass.\n    "
...
class BaseWindow(with_metaclass(_WindowMetaclass, EventDispatcher)):
    ...
    def __init__(self, width=None, height=None, caption=None, resizable=False, style=WINDOW_STYLE_DEFAULT, fullscreen=False, visible=True, vsync=True, file_drops=False, display=None, screen=None, config=None, context=None, mode=None):
        """Create a window.
        ...
        if not config:
            for template_config in [gl.Config(double_buffer=True, depth_size=24), gl.Config(double_buffer=True, depth_size=16),
             None]:
                try:
        ...
        if fullscreen == self._fullscreen:
            if screen is None or screen is self._screen:
                if width is None or width == self._width:
                    if height is None or height == self._height:
                        return
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.ESCAPE:
            if not modifiers & ~(key.MOD_NUMLOCK | key.MOD_CAPSLOCK | key.MOD_SCROLLLOCK):
    ...
    def draw_mouse_cursor(self):
        ...
        """
        if self._mouse_cursor.gl_drawable:
            if self._mouse_visible:
                if self._mouse_in_window:
                    gl.glMatrixMode(gl.GL_PROJECTION)
                    gl.glPushMatrix()
                    gl.glLoadIdentity()
                    gl.glOrtho(0, self.width, 0, self.height, -3000, 300)
                    gl.glMatrixMode(gl.GL_MODELVIEW)
                    gl.glPushMatrix()
                    gl.glLoadIdentity()
...
class FPSDisplay:
    ...
    def draw(self):
        ...
        gl.glPushMatrix()
        gl.glLoadIdentity()
        gl.glOrtho(0, self.window.width, 0, self.window.height, -3000, 3000)
        self.label.draw()
        gl.glPopMatrix()
        gl.glMatrixMode(gl.GL_MODELVIEW)
        gl.glPopMatrix()
