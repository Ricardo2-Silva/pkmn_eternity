...
def main(args=None, *, _wrap_timer=None):
    ...
    if args is None:
        ...
        verbose = 0
        time_unit = None
        units = {'usec':1,  'msec':1000.0,  'sec':1000000.0}
        precision = 3
        for o, a in opts:
            if o in ('-n', '--number'):
                number = int(a)
            ...
            if o in ('-u', '--unit'):
                if a in units:
                    ...
                    print("Unrecognized unit. Please select usec, msec, or sec.", file=(sys.stderr))
                    return 2
                if o in ('-r', '--repeat'):
                    repeat = int(a)
                    if repeat <= 0:
    ...
    try:
        ...
        worst = max(r)
        if worst >= best * 4:
            usec = worst * 1000000.0 / number
            import warnings
            warnings.warn_explicit("The test results are likely unreliable. The worst\ntime (%.*g %s) was more than four times slower than the best time." % (
