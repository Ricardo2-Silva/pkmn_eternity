...
# Embedded file name: service_identity\pyopenssl.py
"""
...
def verify_hostname(connection, hostname):
    ...
    :returns: ``None``
    """
    verify_service_identity(cert_patterns=(extract_ids(connection.get_peer_certificate())),
      obligatory_ids=[
     DNS_ID(hostname)],
      optional_ids=[])
...
def extract_ids(cert):
    ...
    if not ids:
        components = [c[1] for c in cert.get_subject().get_components() if c[0] == b'CN']
        cn = next(iter(components), b'<not given>')
        ids = [DNSPattern(c) for c in components]
        warnings.warn(("Certificate with CN '%s' has no `subjectAltName`, falling back to check for a `commonName` for now.  This feature is being removed by major browsers and deprecated by RFC 2818.  service_identity will remove the support for it in mid-2018." % (
         cn.decode("utf-8"),)),
