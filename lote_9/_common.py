...
# Embedded file name: service_identity\_common.py
"""
Common verification code.
"""
from __future__ import absolute_import, division, print_function
import ipaddress, re, attr
from ._compat import maketrans, text_type
from .exceptions import CertificateError, DNSMismatch, IPAddressMismatch, SRVMismatch, URIMismatch, VerificationError
try:
    import idna
except ImportError:
    idna = None
...
class SRVPattern(object):
    ...
    def __init__(self, pattern):
        ...
        pattern = pattern.strip().translate(_TRANS_TO_LOWER)
        if pattern[0] != 95 or b'.' not in pattern or b'*' in pattern or _is_ip_address(pattern):
...
def _validate_pattern(cert_pattern):
    ...
    if cnt > 1:
        raise CertificateError("Certificate's DNS-ID {0!r} contains too many wildcards.".format(cert_pattern))
    parts = cert_pattern.split(b'.')
    if len(parts) < 3:
        raise CertificateError("Certificate's DNS-ID {0!r} has too few host components for wildcard usage.".format(cert_pattern))
    if b'*' not in parts[0]:
        raise CertificateError("Certificate's DNS-ID {0!r} has a wildcard outside the left-most part.".format(cert_pattern))
    if any(not len(p) for p in parts):
        raise CertificateError("Certificate's DNS-ID {0!r} contains empty parts.".format(cert_pattern))


_TRANS_TO_LOWER = maketrans(b'ABCDEFGHIJKLMNOPQRSTUVWXYZ', b'abcdefghijklmnopqrstuvwxyz')
