...
# Embedded file name: twisted\spread\pb.py
"""
Perspective Broker
...
from twisted.python.compat import unicode, _bytesChr as chr, range, comparable, cmp
...
class Broker(banana.Banana):
    ...
    def serialize(self, object, perspective=None, method=None, args=None, kw=None):
        ...
        if isinstance(object, defer.Deferred):
            object.addCallbacks((self.serialize), (lambda x: x), callbackKeywords={
            ...
            try:
                ...
                if answerRequired:
                    if isinstance(netResult, defer.Deferred):
                        args = (
                         requestID,)
                        netResult.addCallbacks((self._sendAnswer), (self._sendFailureOrError), callbackArgs=args,
                          errbackArgs=args)
                    else:
...
class PBClientFactory(protocol.ClientFactory):
    ...
    def getRootObject(self):
        ...
        """
        if self._broker:
            if not self._broker.disconnected:
