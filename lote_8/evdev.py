...
# Embedded file name: pyglet\input\evdev.py
import os, errno, ctypes, pyglet
...
def _create_joystick(device):
    ...
    for control in device.controls:
        if control._event_type == EV_ABS and control._event_code == ABS_X:
            ...
            if control._event_type == EV_KEY and control._event_code in (BTN_JOYSTICK, BTN_GAMEPAD):
                have_button = True
...
def get_joysticks(display=None):
    return [joystick for joystick in [_create_joystick(device) for device in get_devices(display)] if joystick is not None]
