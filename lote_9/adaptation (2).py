...
# Embedded file name: pyglet\media\drivers\pulse\adaptation.py
import weakref
...
class PulseAudioPlayer(AbstractAudioPlayer):
    ...
    def on_write_needed(self, nbytes, underflow):
        ...
        if self._events:
            if self._time_sync_operation is not None:
                if self._time_sync_operation.is_done:
    ...
    def _get_audio_data(self, nbytes=None):
        ...
        if self._current_audio_data is None:
            ...
            assert _debug("PulseAudioPlayer: Got {} bytes of audio data".format(self._current_audio_data.length))
    ...
    def _schedule_events(self):
        if self._current_audio_data is not None:
            for event in self._current_audio_data.events:
                event_index = self._write_index + event.timestamp * self.source.audio_format.bytes_per_second
    ...
    def _write_to_stream(self, nbytes=None):
        if nbytes is None:
        ...
        elif not _debug("PulseAudioPlayer: Requested to write %d bytes to stream" % nbytes):
            raise AssertionError
        else:
            ...
            if self._clear_write:
                ...
                self._clear_write = False
                if not _debug("PulseAudioPlayer: Clear buffer"):
                    raise AssertionError
            while self._has_audio_data() and nbytes > 0:
                ...
                assert _debug("PulseAudioPlayer: Actually wrote {} bytes to stream".format(consumption))
    ...
    def _handle_underflow(self):
        if not _debug("Player: underflow"):
            raise AssertionError
        elif self._has_audio_data():
            self._write_to_stream()
        else:
            self._add_event_at_write_index("on_eos")

    def _process_events(self):
        if not _debug("PulseAudioPlayer: Process events"):
            raise AssertionError
        else:
            if not self._events:
    ...
    def _add_event_at_write_index(self, event_name):
        ...
        self._events.append((self._write_index, MediaEvent(0.0, event_name)))

    def delete(self):
        if not _debug("Delete PulseAudioPlayer"):
            raise AssertionError
        else:
            ...
            if driver.mainloop is None:
                assert _debug("PulseAudioDriver already deleted. PulseAudioPlayer could not clean up properly.")
                return
            if self._time_sync_operation is not None:
        ...
        with self.stream:
            if self.stream.is_corked:
                ...
                if not self._has_audio_data():
                    ...
                    if not _debug("PulseAudioPlayer: Triggered stream for immediate playback"):
                        raise AssertionError
