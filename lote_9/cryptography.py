...
# Embedded file name: service_identity\cryptography.py
"""
`cryptography.x509 <https://github.com/pyca/cryptography>`_-specific code.
"""
from __future__ import absolute_import, division, print_function
import warnings
from cryptography.x509 import DNSName, ExtensionOID, IPAddress, NameOID, ObjectIdentifier, OtherName, UniformResourceIdentifier
from cryptography.x509.extensions import ExtensionNotFound
from pyasn1.codec.der.decoder import decode
from pyasn1.type.char import IA5String
from ._common import DNS_ID, CertificateError, DNSPattern, IPAddress_ID, IPAddressPattern, SRVPattern, URIPattern, verify_service_identity
...
def verify_certificate_hostname(certificate, hostname):
    ...
    :returns: ``None``
    """
    verify_service_identity(cert_patterns=(extract_ids(certificate)),
      obligatory_ids=[
     DNS_ID(hostname)],
      optional_ids=[])
...
def verify_certificate_ip_address(certificate, ip_address):
    ...
    verify_service_identity(cert_patterns=(extract_ids(certificate)),
      obligatory_ids=[
     IPAddress_ID(ip_address)],
      optional_ids=[])


ID_ON_DNS_SRV = ObjectIdentifier("1.3.6.1.5.5.7.8.7")
...
def extract_ids(cert):
    ...
    try:
        ...
        ids.extend([DNSPattern(name.encode("utf-8")) for name in ext.value.get_values_for_type(DNSName)])
        ids.extend([URIPattern(uri.encode("utf-8")) for uri in ext.value.get_values_for_type(UniformResourceIdentifier)])
    ...
    if not ids:
        cns = [n.value for n in cert.subject.get_attributes_for_oid(NameOID.COMMON_NAME)]
        cn = next(iter(cns), b'<not given>')
        ids = [DNSPattern(n.encode("utf-8")) for n in cns]
        warnings.warn("Certificate with CN {!r} has no `subjectAltName`, falling back to check for a `commonName` for now.  This feature is being removed by major browsers and deprecated by RFC 2818.".format(cn), SubjectAltNameWarning)
    return ids
