...
# Embedded file name: pyglet\window\win32\__init__.py
from ctypes import *
...
class Win32Window(BaseWindow):
    ...
    def _create(self):
        if self._fullscreen:
            ...
            styles = {(self.WINDOW_STYLE_DEFAULT): (WS_OVERLAPPEDWINDOW, 0), 
        ...
        self._hwnd = self._hwnd or _user32.CreateWindowExW(self._ex_ws_style, self._window_class.lpszClassName, "", self._ws_style, CW_USEDEFAULT, CW_USEDEFAULT, width, height, 0, 0, self._window_class.hInstance, 0)
        ...
        if self._file_drops:
            ...
            raw_keyboard = RAWINPUTDEVICE(1, 6, 0, self._view_hwnd)
    ...
    def flip(self):
        ...
        if self._fullscreen or self._always_dwm or self._dwm_composition_enabled():
            if self._interval:
                _dwmapi.DwmFlush()
        self.context.flip()
    ...
    def set_exclusive_mouse(self, exclusive=True):
        if self._exclusive_mouse == exclusive:
            if self._exclusive_mouse_focus == self._has_focus:
                return
            else:
    ...
    def set_exclusive_keyboard(self, exclusive=True):
        if self._exclusive_keyboard == exclusive:
            if self._exclusive_keyboard_focus == self._has_focus:
                return
        elif exclusive and self._has_focus:
    ...
    def _get_modifiers(self, key_lParam=0):
        ...
        if key_lParam:
            ...
            if _user32.GetKeyState(VK_MENU) < 0:
                modifiers |= key.MOD_ALT
        return modifiers
        ...
        if inp.header.dwType == RIM_TYPEMOUSE:
            if not self._exclusive_mouse:
                ...
                if rmouse.usButtonFlags & RI_MOUSE_LEFT_BUTTON_DOWN:
                    self.dispatch_event("on_mouse_press", 0, 0, mouse.LEFT, self._get_modifiers())
                    self._exclusive_mouse_buttons |= mouse.LEFT
                if rmouse.usButtonFlags & RI_MOUSE_LEFT_BUTTON_UP:
                    self.dispatch_event("on_mouse_release", 0, 0, mouse.LEFT, self._get_modifiers())
                    self._exclusive_mouse_buttons &= ~mouse.LEFT
                if rmouse.usButtonFlags & RI_MOUSE_RIGHT_BUTTON_DOWN:
                    self.dispatch_event("on_mouse_press", 0, 0, mouse.RIGHT, self._get_modifiers())
                    self._exclusive_mouse_buttons |= mouse.RIGHT
                if rmouse.usButtonFlags & RI_MOUSE_RIGHT_BUTTON_UP:
                    self.dispatch_event("on_mouse_release", 0, 0, mouse.RIGHT, self._get_modifiers())
                    self._exclusive_mouse_buttons &= ~mouse.RIGHT
                if rmouse.usButtonFlags & RI_MOUSE_MIDDLE_BUTTON_DOWN:
                    self.dispatch_event("on_mouse_press", 0, 0, mouse.MIDDLE, self._get_modifiers())
                    self._exclusive_mouse_buttons |= mouse.MIDDLE
                if rmouse.usButtonFlags & RI_MOUSE_MIDDLE_BUTTON_UP:
                    self.dispatch_event("on_mouse_release", 0, 0, mouse.MIDDLE, self._get_modifiers())
                    self._exclusive_mouse_buttons &= ~mouse.MIDDLE
                if rmouse.usButtonFlags & RI_MOUSE_WHEEL:
                    delta = SHORT(rmouse.usButtonData).value
                    self.dispatch_event("on_mouse_scroll", 0, 0, 0, delta / float(WHEEL_DELTA))
    ...
    def _event_size(self, msg, wParam, lParam):
        if not self._dc:
            ...
            if wParam == SIZE_MINIMIZED:
                ...
                self._width, self._height = self._fullscreen or w, h
            self._update_view_location(self._width, self._height)
            self.switch_to()
            self.dispatch_event("on_resize", self._width, self._height)
            return 0
