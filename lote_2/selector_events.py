...
class BaseSelectorEventLoop(base_events.BaseEventLoop):
    ...
    def _make_ssl_transport(self, rawsock, protocol, sslcontext, waiter=None, *, server_side=False, server_hostname=None, extra=None, server=None):
        if not sslproto._is_sslproto_available():
            return self._make_legacy_ssl_transport(rawsock,
    ...
    def _start_serving(self, protocol_factory, sock, sslcontext=None, server=None, backlog=100):
        self._add_reader(sock.fileno(), self._accept_connection, protocol_factory, sock, sslcontext, server, backlog)
            ...
            try:
                ...
                if exc.errno in (errno.EMFILE, errno.ENFILE,
                 errno.ENOBUFS, errno.ENOMEM):
                    ...
                    self.call_later(constants.ACCEPT_RETRY_DELAY, self._start_serving, protocol_factory, sock, sslcontext, server, backlog)
                else:
                    raise
            else:
                extra = {"peername": addr}
                accept = self._accept_connection2(protocol_factory, conn, extra, sslcontext, server)
    ...
    def sock_connect(self, sock, address):
        ...
        if not hasattr(socket, "AF_UNIX") or sock.family != socket.AF_UNIX:
            resolved = base_events._ensure_resolved(address,
...
class _SelectorSocketTransport(_SelectorTransport):
    ...
    def write(self, data):
        if not isinstance(data, (bytes, bytearray, memoryview)):
            raise TypeError("data argument must be a bytes-like object, not %r" % type(data).__name__)
        else:
            if self._eof:
...
class _SelectorSslTransport(_SelectorTransport):
    ...
    def __init__(self, loop, rawsock, protocol, sslcontext, waiter=None, server_side=False, server_hostname=None, extra=None, server=None):
        if ssl is None:
            ...
            wrap_kwargs = {'server_side':server_side, 
    ...
    def _on_handshake(self, start_time):
        try:
            ...
            if self._loop.get_debug():
                logger.warning("%r: SSL handshake failed", self,
                  exc_info=True)
            self._loop._remove_reader(self._sock_fd)
            self._loop._remove_writer(self._sock_fd)
            self._sock.close()
        ...
        if hasattr(self._sslcontext, "check_hostname") or self._server_hostname:
            if self._sslcontext.verify_mode != ssl.CERT_NONE:
                try:
                    ssl.match_hostname(peercert, self._server_hostname)
                except Exception as exc:
                    if self._loop.get_debug():
                        logger.warning("%r: SSL handshake failed on matching the hostname", self,
                          exc_info=True)
                    self._sock.close()
                    self._wakeup_waiter(exc)
                    return

        self._extra.update(peercert=peercert, cipher=(self._sock.cipher()),
