...
# Embedded file name: twisted\internet\defer.py
"""
...
import attr, traceback, types, warnings
...
@deprecated((Version("Twisted", 17, 1, 0)), replacement="twisted.internet.defer.Deferred.addTimeout")
...
class Deferred:
    ...
    def addErrback(self, errback, *args, **kw):
        ...
        See L{addCallbacks}.
        """
        return self.addCallbacks(passthru, errback, errbackArgs=args,
          errbackKeywords=kw)
    ...
    def addBoth(self, callback, *args, **kw):
        ...
        See L{addCallbacks}.
        """
        return self.addCallbacks(callback, callback, callbackArgs=args,
          errbackArgs=args,
          callbackKeywords=kw,
          errbackKeywords=kw)
    ...
    def __del__(self):
        ...
        if self.failResult is not None:
            ...
            if debugInfo:
                ...
                format = None
            log.failure(format, (self.failResult),
              debugInfo=debugInfo)
...
class DeferredList(Deferred):
    ...
    def _cbDeferred(self, result, index, succeeded):
        ...
        if self.called or succeeded == SUCCESS:
...
class waitForDeferred:
    ...
    def __init__(self, d):
        warnings.warn("twisted.internet.defer.waitForDeferred was deprecated in Twisted 15.0.0; please use twisted.internet.defer.inlineCallbacks instead",
          DeprecationWarning,
          stacklevel=2)
...
class DeferredSemaphore(_ConcurrencyPrimitive):
    ...
    def acquire(self):
        ...
        if not self.tokens >= 0:
            raise AssertionError("Internal inconsistency??  tokens should never be negative")
        else:
            d = Deferred(canceller=(self._cancelAcquire))
            if not self.tokens:
                self.waiting.append(d)
