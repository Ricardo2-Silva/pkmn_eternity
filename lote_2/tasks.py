...
__all__ = [
 ...
 'wait', 
 'wait_for', 'as_completed', 'sleep', 'async', 
            ...
            try:
                ...
                if blocking is not None:
                    if result._loop is not self._loop:
                        self._loop.call_soon(self._step, RuntimeError("Task {!r} got Future {!r} attached to a different loop".format(self, result)))
                    elif blocking:
                        if result is self:
                        ...
                        if self._must_cancel:
                            if self._fut_waiter.cancel():
                                ...
                                self._loop.call_soon(self._step, RuntimeError("yield was used instead of yield from in task {!r} with {!r}".format(self, result)))
                elif result is None:
                    self._loop.call_soon(self._step)
                elif inspect.isgenerator(result):
                    self._loop.call_soon(self._step, RuntimeError("yield was used instead of yield from for generator in task {!r} with {!r}".format(self, result)))
                else:
...
def async_(coro_or_future, *, loop=None):
    ...
    """
    warnings.warn("asyncio.async() function is deprecated, use ensure_future()", DeprecationWarning,
      stacklevel=2)
