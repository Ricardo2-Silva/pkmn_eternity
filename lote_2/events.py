...
# Embedded file name: asyncio\events.py
...
__all__ = [
 ...
 '_set_running_loop', 
 '_get_running_loop']
import functools, inspect, os, reprlib, socket, subprocess, sys, threading, traceback
from . import compat
...
def extract_stack(f=None, limit=None):
    ...
    stack = traceback.StackSummary.extract((traceback.walk_stack(f)), limit=limit,
      lookup_lines=False)
    stack.reverse()
    return stack
...
class Handle:
    __doc__ = "Object returned by callback registration methods."
    __slots__ = ('_callback', '_args', '_cancelled', '_loop', '_source_traceback',
                 '_repr', '__weakref__')
...
class TimerHandle(Handle):
    __doc__ = "Object returned by timed callback registration methods."
    __slots__ = [
     "_scheduled", "_when"]
...
class AbstractServer:
    __doc__ = "Abstract server returned by create_server()."
...
class AbstractEventLoopPolicy:
    __doc__ = "Abstract policy for accessing the event loop."
...
class BaseDefaultEventLoopPolicy(AbstractEventLoopPolicy):
    ...
    def get_event_loop(self):
        ...
        if self._local._loop is None:
            if not self._local._set_called:
...
def set_event_loop_policy(policy):
    ...
    if not policy is None:
        if not isinstance(policy, AbstractEventLoopPolicy):
            raise AssertionError
