...
def _handle_rsa_enc_dec_error(backend, key):
    ...
    if isinstance(key, _RSAPublicKey):
        raise ValueError("Data too long for key size. Encrypt less data or use a larger key size.", errors)
    else:
...
def _rsa_sig_sign(backend, padding, algorithm, private_key, data):
    pkey_ctx = _rsa_sig_setup(backend, padding, algorithm, private_key, data, backend._lib.EVP_PKEY_sign_init)
...
def _rsa_sig_verify(backend, padding, algorithm, public_key, signature, data):
    pkey_ctx = _rsa_sig_setup(backend, padding, algorithm, public_key, data, backend._lib.EVP_PKEY_verify_init)
