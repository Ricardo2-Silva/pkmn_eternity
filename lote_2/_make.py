...
# Embedded file name: attr\_make.py
    ...
    if these is not None:
        ...
        if auto_attribs is True:
            ...
            if len(unannotated) > 0:
                ...
                ca_list = sorted(((name, attr) for name, attr in cd.items() if isinstance(attr, _CountingAttr)),
...
class _ClassBuilder(object):
    ...
    __slots__ = ('_attr_names', '_attrs', '_base_attr_map', '_base_names', '_cache_hash',
                 '_cls', '_cls_dict', '_delete_attribs', '_frozen', '_has_post_init',
                 '_is_exc', '_on_setattr', '_slots', '_weakref_slot', '_has_own_setattr',
                 '_has_custom_setattr')

    def __init__(self, cls, these, slots, frozen, weakref_slot, getstate_setstate, auto_attribs, kw_only, cache_hash, is_exc, collect_by_mro, on_setattr, has_custom_setattr):
        attrs, base_attrs, base_map = _transform_attrs(cls, these, auto_attribs, kw_only, collect_by_mro)
        self._cls = cls
    ...
    def _patch_original_class(self):
        ...
        if not self._has_own_setattr:
            if getattr(cls, "__attrs_own_setattr__", False):
    ...
    def _create_slots_class(self):
        ...
        if self._weakref_slot:
            if "__weakref__" not in getattr(self._cls, "__slots__", ()):
                if "__weakref__" not in names:
                    if not weakref_inherited:
                        names += ('__weakref__', )
    ...
    def _make_getstate_setstate(self):
        ...
        def slots_setstate(self, state):
            ...
            Automatically created by attrs.
            """
            _ClassBuilder__bound_setattr = _obj_setattr.__get__(self, Attribute)
            for name, value in zip(state_attr_names, state):
                _ClassBuilder__bound_setattr(name, value)

            if hash_caching_enabled:
                _ClassBuilder__bound_setattr(_hash_cache_field, None)
    ...
    def add_init(self):
        self._cls_dict["__init__"] = self._add_method_dunders(_make_init(self._cls, self._attrs, self._has_post_init, self._frozen, self._slots, self._cache_hash, self._base_attr_map, self._is_exc, self._on_setattr is not None and self._on_setattr is not setters.NO_OP))
        return self
...
def attrs(maybe_cls=None, these=None, repr_ns=None, repr=None, cmp=None, hash=None, init=None, slots=False, frozen=False, weakref_slot=True, str=False, auto_attribs=False, kw_only=False, cache_hash=False, auto_exc=False, eq=None, order=None, auto_detect=False, collect_by_mro=False, getstate_setstate=None, on_setattr=None):
    r"""
    ...
    def wrapParse error at or near `POP_JUMP_IF_TRUE' instruction at offset 350_352
...
def _make_init(cls, attrs, post_init, frozen, slots, cache_hash, base_attr_map, is_exc, has_global_on_setattr):
    if frozen:
        if has_global_on_setattr:
    ...
    for a in attrs:
        if not a.init:
            ...
            if has_global_on_setattr and a.on_setattr is not setters.NO_OP or _is_slot_attr(a.name, base_attr_map):
                needs_cached_setattr = True

    unique_filename = _generate_unique_filename(cls, "init")
    script, globs, annotations = _attrs_to_init_script(filtered_attrs, frozen, slots, post_init, cache_hash, base_attr_map, is_exc, needs_cached_setattr, has_global_on_setattr)
    locs = {}
    bytecode = compile(script, unique_filename, "exec")
...
def _attrs_to_init_script(attrs, frozen, slots, post_init, cache_hash, base_attr_map, is_exc, needs_cached_setattr, has_global_on_setattr):
    """
            ...
            if a.init is False:
                if has_factory:
                    ...
                    if a.converter is not None:
                        lines.append(fmt_setter_with_converter(attr_name, init_factory_name + "(%s)" % (maybe_self,), has_on_setattr))
                        conv_name = _init_converter_pat % (a.name,)
                        names_for_globals[conv_name] = a.converter
                    else:
                        lines.append(fmt_setter(attr_name, init_factory_name + "(%s)" % (maybe_self,), has_on_setattr))
                    names_for_globals[init_factory_name] = a.default.factory
                elif a.converter is not None:
                    lines.append(fmt_setter_with_converter(attr_name, "attr_dict['%s'].default" % (attr_name,), has_on_setattr))
                    conv_name = _init_converter_pat % (a.name,)
                    names_for_globals[conv_name] = a.converter
                else:
                    lines.append(fmt_setter(attr_name, "attr_dict['%s'].default" % (attr_name,), has_on_setattr))
            elif a.default is not NOTHING:
                pass
            if not has_factory:
    ...
    if kw_only_args:
        ...
        args += "{leading_comma}*, {kw_only_args}".format(leading_comma=(", " if args else ""),
...
class Attribute(object):
    ...
    __slots__ = ('name', 'default', 'validator', 'repr', 'eq', 'order', 'hash', 'init',
                 'metadata', 'type', 'converter', 'kw_only', 'inherited', 'on_setattr')

    def __init__(self, name, default, validator, repr, cmp, hash, init, inherited, metadata=None, type=None, converter=None, kw_only=False, eq=None, order=None, on_setattr=None):
        eq, order = _determine_eq_order(cmp, eq, order, True)
...
_a = [Attribute(name=name, default=NOTHING, validator=None, repr=True, cmp=None, eq=True, order=False, hash=(name != "metadata"), init=True, inherited=False) for name in Attribute.__slots__]
Attribute = _add_hash(_add_eq(_add_repr(Attribute, attrs=_a),
  attrs=[a for a in _a if a.name != "inherited"]),
  attrs=[a for a in _a if a.hash if a.name != "inherited"])

class _CountingAttr(object):
    __doc__ = "\n    Intermediate representation of attributes that uses a counter to preserve\n    the order in which the attributes have been defined.\n\n    *Internal* data structure of the attrs library.  Running into is most\n    likely the result of a bug like a forgotten `@attr.s` decorator.\n    "
    __slots__ = ('counter', '_default', 'repr', 'eq', 'order', 'hash', 'init', 'metadata',
                 '_validator', 'converter', 'type', 'kw_only', 'on_setattr')
    __attrs_attrs__ = tuple(Attribute(name=name, default=NOTHING, validator=None, repr=True, cmp=None, hash=True, init=True, kw_only=False, eq=True, order=False, inherited=False, on_setattr=None) for name in ('counter',
                                                                                                                                                                                                                 '_default',
                                                                                                                                                                                                                 'repr',
                                                                                                                                                                                                                 'eq',
                                                                                                                                                                                                                 'order',
    ...
    def __init__(self, default, validator, repr, cmp, hash, init, converter, metadata, type, kw_only, eq, order, on_setattr):
...
def make_class(name, attrs, bases=(
 object,), **attributes_arguments):
    ...
    attributes_arguments["eq"], attributes_arguments["order"] = _determine_eq_order(cmp, attributes_arguments.get("eq"), attributes_arguments.get("order"), True)
