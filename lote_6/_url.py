...
# Embedded file name: hyperlink\_url.py
...
from typing import Any, Callable, Dict, Iterable, Iterator, List, Mapping, Optional, Sequence, Text, Tuple, Type, TypeVar, Union, cast
...
NoneType = type(None)
QueryPairs = Tuple[(Tuple[(Text, Optional[Text])], ...)]
...
_AUTHORITY_RE = re.compile("^(?:(?P<userinfo>[^@/?#]*)@)?(?P<host>(?:\\[(?P<ipv6_host>[^[\\]/?#]*)\\])|(?P<plain_host>[^:/?#[\\]]*)|(?P<bad_host>.*?))?(?::(?P<port>.*))?$")
_HEX_CHAR_MAP = dict([((a + b).encode("ascii"), unichr(int(a + b, 16)).encode("charmap")) for a in string.hexdigits for b in iter((string.hexdigits))])
...
_UNRESERVED_DECODE_MAP = dict([(k, v) for k, v in _HEX_CHAR_MAP.items() if v.decode("ascii", "replace") in _UNRESERVED_CHARS])
...
def _encode_reserved(text, maximal=True):
    ...
    if maximal:
        ...
        return "".join([_UNRESERVED_QUOTE_MAP[t] if t in _UNRESERVED_CHARS else t for t in text])
...
def _encode_schemeless_path_part(text, maximal=True):
    ...
    if maximal:
        ...
        return "".join([_SCHEMELESS_PATH_PART_QUOTE_MAP[t] if t in _SCHEMELESS_PATH_DELIMS else t for t in text])
    ...
    if not text_parts:
        ...
        if rooted:
            ...
            if has_scheme:
                ...
                encoded_parts = [
                 _encode_schemeless_path_part(text_parts[0])]
...
def _encode_query_key(text, maximal=True):
    ...
    if maximal:
        ...
        return "".join([_QUERY_KEY_QUOTE_MAP[t] if t in _QUERY_KEY_DELIMS else t for t in text])
...
def _encode_query_value(text, maximal=True):
    ...
    if maximal:
        ...
        return "".join([_QUERY_VALUE_QUOTE_MAP[t] if t in _QUERY_VALUE_DELIMS else t for t in text])
...
def _encode_fragment_part(text, maximal=True):
    ...
    if maximal:
        ...
        return "".join([_FRAGMENT_QUOTE_MAP[t] if t in _FRAGMENT_DELIMS else t for t in text])
...
def _encode_userinfo_part(text, maximal=True):
    ...
    if maximal:
        ...
        return "".join([_USERINFO_PART_QUOTE_MAP[t] if t in _USERINFO_DELIMS else t for t in text])
...
class URLParseError(ValueError):
    __doc__ = "Exception inheriting from :exc:`ValueError`, raised when failing to\n    parse a URL. Mostly raised on invalid ports and IPv6 addresses.\n    "
    return
...
class URL(object):
            ...
            if scheme is None:
                ...
                if port is None:
                    if scheme is not None:
                        ...
                        if self._scheme:
                            raise _SCHEME_RE.match(self._scheme) or ValueError('invalid scheme: %r. Only alphanumeric, "+", "-", and "." allowed. Did you meant to call %s.from_text()?' % (
                    ...
                    self._path = tuple(_textcheck("path segment", segment, "/?#") for segment in path)
                    self._query = tuple((_textcheck("query parameter name", k, "&=#"), _textcheck("query parameter value", v, "&#", nullable=True)) for k, v in iter_pairs(query))
                    ...
                    will_have_authority = self._host or self._port and self._port != SCHEME_PORT_MAP.get(scheme)
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            ...
            for attr in ('scheme', 'userinfo', 'host', 'query', 'fragment', 'port',
                         'uses_netloc', 'rooted'):
                if getattr(self, attr) != getattr(other, attr):
                    return False

            if self.path == other.path or self.path in _ROOT_PATHS and other.path in _ROOT_PATHS:
                return True
            return False
    ...
    def replace(self, scheme=_UNSET, host=_UNSET, path=_UNSET, query=_UNSET, fragment=_UNSET, port=_UNSET, rooted=_UNSET, userinfo=_UNSET, uses_netloc=_UNSET):
        ...
        if scheme is not _UNSET:
            if scheme != self.scheme:
                uses_netloc = None
        return self.__class__(scheme=(_optional(scheme, self.scheme)),
    ...
    def from_text(cls, text):
        ...
        if au_gs["bad_host"]:
            raise URLParseError("invalid host %r in url: %r" % (au_gs["bad_host"], text))
        else:
            ...
            if port is not None:
                ...
                if gs["path"]:
                    ...
                    rooted = bool(au_text)
                if gs["query"]:
                    query = tuple(qe.split("=", 1) if "=" in qe else (qe, None) for qe in gs["query"].split("&"))
            else:
                query = ()
        ...
        * Encode any stray percent signs (`%`) in percent-encoded
        ...
        if scheme:
            ...
            def _dec_unres(target):
                return _decode_unreserved(target,
                  normalize_case=True, encode_stray_percents=percents)
            ...
            if path:
                if self.path:
                    ...
                    kw["path"] = ('', )
        if query:
            kw["query"] = [(_dec_unres(k), _dec_unres(v) if v else v) for k, v in self.query]
        if fragment:
            kw["fragment"] = _dec_unres(self.fragment)
        if userinfo:
            kw["userinfo"] = ":".join([_dec_unres(p) for p in self.userinfo.split(":", 1)])
    ...
    def to_uri(self):
        ...
        Returns:
            URL: A new instance with its path segments, query parameters, and
                ...
                US-ASCII range.
        """
        new_userinfo = ":".join([_encode_userinfo_part(p) for p in self.userinfo.split(":", 1)])
        new_path = _encode_path_parts((self.path),
          has_scheme=(bool(self.scheme)), rooted=False, maximal=True)
        ...
        return self.replace(userinfo=new_userinfo,
          host=new_host,
          path=new_path,
          query=(tuple([(_encode_query_key(k, maximal=True), _encode_query_value(v, maximal=True) if v is not None else None) for k, v in self.query])),
    ...
    def to_iri(self):
        ...
        """
        new_userinfo = ":".join([_decode_userinfo_part(p) for p in self.userinfo.split(":", 1)])
        host_text = _decode_host(self.host)
        return self.replace(userinfo=new_userinfo,
          host=host_text,
          path=[_decode_path_part(segment) for segment in self.path],
          query=(tuple((_decode_query_key(k), _decode_query_value(v) if v is not None else None) for k, v in self.query)),
    ...
    def to_text(self, with_password=False):
        ...
        path = "/".join(_encode_path_parts((self.path),
          rooted=(self.rooted),
...
class DecodedURL(object):
    ...
    def normalize(self, scheme=True, host=True, path=True, query=True, fragment=True, userinfo=True, percents=True):
        ...
        """
        return self.__class__(self._url.normalize(scheme, host, path, query, fragment, userinfo, percents))
    ...
    def query(self):
        if not hasattr(self, "_query"):
            self._query = cast(QueryPairs, tuple(tuple(_percent_decode(x, raise_subencoding_exc=True) if x is not None else None for x in (k, v)) for k, v in self._url.query))
        return self._query
    ...
    def replace(self, scheme=_UNSET, host=_UNSET, path=_UNSET, query=_UNSET, fragment=_UNSET, port=_UNSET, rooted=_UNSET, userinfo=_UNSET, uses_netloc=_UNSET):
        ...
        if path is not _UNSET:
            ...
            if userinfo is not _UNSET:
                if len(userinfo) > 2:
                    raise ValueError('userinfo expected sequence of ["user"] or ["user", "password"], got %r' % (
                     userinfo,))
