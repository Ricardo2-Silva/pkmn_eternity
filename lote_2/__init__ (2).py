...
# Embedded file name: bcrypt\__init__.py
from __future__ import absolute_import
from __future__ import division
import os, re, warnings, six
from . import _bcrypt
...
def gensalt(rounds=12, prefix=b'2b'):
    ...
    return b'$' + prefix + b'$' + ("%2.2u" % rounds).encode("ascii") + b'$' + _bcrypt.ffi.string(output)
...
def kdf(password, salt, desired_key_bytes, rounds, ignore_few_rounds=False):
    if isinstance(password, six.text_type) or isinstance(salt, six.text_type):
        raise TypeError("Unicode-objects must be encoded before hashing")
    elif len(password) == 0 or len(salt) == 0:
        ...
        if rounds < 50:
            if not ignore_few_rounds:
                warnings.warn(("Warning: bcrypt.kdf() called with only {0} round(s). This few is not secure: the parameter is linear, like PBKDF2.".format(rounds)),
                  UserWarning,
                  stacklevel=2)
    key = _bcrypt.ffi.new("uint8_t[]", desired_key_bytes)
    res = _bcrypt.lib.bcrypt_pbkdf(password, len(password), salt, len(salt), key, len(key), rounds)
