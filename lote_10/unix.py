...
# Embedded file name: twisted\internet\unix.py
"""
    ...
    class _SendmsgMixin(object):
        ...
        """
            ...
            for cmsgLevel, cmsgType, cmsgData in ancillary:
                if cmsgLevel == socket.SOL_SOCKET and cmsgType == sendmsg.SCM_RIGHTS:
                    ...
                    log.msg(format="%(protocolName)s (on %(hostAddress)r) received unsupported ancillary data (level=%(cmsgLevel)r, type=%(cmsgType)r) from %(peerAddress)r.",
        ...
        """
            ...
            if interfaces.IFileDescriptorReceiver.providedBy(self.protocol):
                ...
                log.msg(format="%(protocolName)s (on %(hostAddress)r) does not provide IFileDescriptorReceiver; closing file descriptor received (from %(peerAddress)r).",
                  hostAddress=(self.getHost()),
                  peerAddress=(self.getPeer()),
                  protocolName=(self._getLogPrefix(self.protocol)))
                for fd in fds:
...
class Port(_UNIXPort, tcp.Port):
    ...
    def __init__(self, fileName, factory, backlog=50, mode=438, reactor=None, wantPID=0):
        tcp.Port.__init__(self, (self._buildAddr(fileName).name), factory, backlog,
          reactor=reactor)
        self.mode = mode
...
class Client(_SendmsgMixin, tcp.BaseClient):
    __doc__ = "A client for Unix sockets."
    addressFamily = socket.AF_UNIX
    socketType = socket.SOCK_STREAM
    ...
    def __init__(self, filename, connector, reactor=None, checkPID=0):
        ...
        if checkPID:
            if not lockfile.isLocked(filename + b'.lock'):
