...
# Embedded file name: multiprocessing\util.py
import os, itertools, sys, weakref, atexit, threading
...
class Finalize(object):
    ...
    def __init__(self, obj, callback, args=(), kwargs=None, exitpriority=None):
        if not exitpriority is None:
            if not type(exitpriority) is int:
...
def spawnv_passfds(path, args, passfds):
    ...
    try:
        return _posixsubprocess.fork_exec(args, [os.fsencode(path)], True, passfds, None, None, -1, -1, -1, -1, -1, -1, errpipe_read, errpipe_write, False, False, None)
    finally:
