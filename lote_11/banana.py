...
# Embedded file name: twisted\spread\banana.py
"""
...
def int2b128(integer, stream):
    if integer == 0:
        ...
        raise AssertionError("can only encode positive integers")
    while integer:
...
class Banana(protocol.Protocol, styles.Ephemeral):
    ...
    def dataReceived(self, chunk):
        ...
        while buffer:
            if not self.buffer != buffer:
                raise AssertionError("This ain't right: %s %s" % (repr(self.buffer), repr(buffer)))
            else:
                self.buffer = buffer
                pos = 0
    ...
    def _encode(self, obj, write):
        if isinstance(obj, (list, tuple)):
            ...
            if self.currentDialect == b'pb':
                if obj in self.outgoingSymbols:
