...
if sys.platform != "win32":
    ...
    import _winapi, itertools, msvcrt, os, socket, subprocess, tempfile, warnings
    __all__ = [
     'socketpair', 'pipe', 'Popen', 'PIPE', 'PipeHandle']
    BUFSIZE = 8192
...
def pipe(*, duplex=False, overlapped=(True, True), bufsize=BUFSIZE):
    ...
    address = tempfile.mktemp(prefix=("\\\\.\\pipe\\python-pipe-%d-%d-" % (
...
class PipeHandle:
    ...
    def __del__(self):
        if self._handle is not None:
            warnings.warn(("unclosed %r" % self), ResourceWarning, source=self)
            self.close()
...
class Popen(subprocess.Popen):
    ...
    def __init__(self, args, stdin=None, stdout=None, stderr=None, **kwds):
        if not not kwds.get("universal_newlines"):
