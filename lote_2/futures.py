...
# Embedded file name: asyncio\futures.py
"""A Future class similar to the one in PEP 3148."""
__all__ = [
 'CancelledError', 'TimeoutError', 'InvalidStateError', 
 'Future', 'wrap_future', 
 'isfuture']
import concurrent.futures, logging, sys, traceback
    ...
    if compat.PY34:
        ...
        def __del__(self):
            if not self._log_traceback:
                return
            exc = self._exception
            context = {'message':"%s exception was never retrieved" % (self.__class__.__name__), 
             'exception':exc, 
             'future':self}
...
def _chain_future(source, destination):
    ...
    if not isfuture(source):
        if not isinstance(source, concurrent.futures.Future):
            raise TypeError("A future is required for source argument")
    if not isfuture(destination):
        if not isinstance(destination, concurrent.futures.Future):
