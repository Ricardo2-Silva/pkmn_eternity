...
# Embedded file name: pyglet\input\x11_xinput.py
            ...
            if cls_class == xi.KeyClass:
                ...
                if cls_class == xi.ValuatorClass:
                    ...
                    for i in range(num_axes):
                        axis = axes[i]
                        if mode == xi.Absolute:
                            self.axes.append(AbsoluteAxis(("axis%d" % i), min=(axis.min_value),
                              max=(axis.max_value)))
...
class XInputWindowEventDispatcher:
    ...
    def get_dispatcher(window):
        try:
            dispatcher = window._XInputWindowEventDispatcher__xinput_window_event_dispatcher
        except AttributeError:
            dispatcher = window._XInputWindowEventDispatcher__xinput_window_event_dispatcher = XInputWindowEventDispatcher(window)
    ...
    def open_device(self, device_id, device, responder):
        ...
        for i in range(device.num_classes):
            ...
            if class_info.input_class == xi.KeyClass:
                ...
                continue
            if class_info.input_class == xi.FocusClass:
                continue
                if class_info.input_class == xi.OtherClass:
                    pass
