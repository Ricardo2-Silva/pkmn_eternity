...
# Embedded file name: lxml\html\formfill.py
...
def _find_elements_for_name(form, name, error):
    if name is None:
        ...
        if name.startswith("#"):
            ...
            if not els:
                return
            isinstance(error, (list, tuple)) or (yield (
             els[0], error))
            return
