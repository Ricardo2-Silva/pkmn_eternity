    ...
    def parse_config_files(self, filenames=None):
        ...
        if sys.prefix != sys.base_prefix:
            ignore_options = ['install-base', 'install-platbase', 'install-lib', 
             'install-platlib', 
             'install-purelib', 'install-headers', 
             'install-scripts', 
             'install-data', 'prefix', 'exec-prefix', 
             'home', 
             'user', 'root']
        else:
            ignore_options = []
    ...
    def _show_help(self, parser, global_options=1, display_options=1, commands=[]):
        ...
        for command in self.commands:
            ...
            if hasattr(klass, "help_options"):
                if isinstance(klass.help_options, list):
...
class DistributionMetadata:
    ...
    _METHOD_BASENAMES = ('name', 'version', 'author', 'author_email', 'maintainer',
                         ...
                         'keywords', 'platforms', 'fullname', 'contact', 'contact_email',
                         'classifiers', 'download_url', 'provides', 'requires', 'obsoletes')
