...
# Embedded file name: distutils\command\build_py.py
...
class build_py(Command):
    ...
    def find_data_files(self, package, src_dir):
        ...
        for pattern in globs:
            ...
            files.extend([fn for fn in filelist if fn not in files if os.path.isfile(fn)])
    ...
    def get_outputs(self, include_bytecode=1):
        ...
        for package, module, module_file in modules:
            ...
            if include_bytecode:
                if self.compile:
                    outputs.append(importlib.util.cache_from_source(filename,
                      optimization=""))
                if self.optimize > 0:
                    outputs.append(importlib.util.cache_from_source(filename,
                      optimization=(self.optimize)))

        outputs += [os.path.join(build_dir, filename) for package, src_dir, build_dir, filenames in self.data_files for filename in iter(filenames)]
        return outputs
    ...
    def byte_compile(self, files):
        if sys.dont_write_bytecode:
            ...
            if self.compile:
                byte_compile(files, optimize=0, force=(self.force),
                  prefix=prefix,
                  dry_run=(self.dry_run))
            if self.optimize > 0:
                byte_compile(files, optimize=(self.optimize), force=(self.force),
                  prefix=prefix,
                  dry_run=(self.dry_run))
