...
# Embedded file name: twisted\python\log.py
"""
...
def err(_stuff=None, _why=None, **kw):
    ...
    elif isinstance(_stuff, failure.Failure):
...
if "theLogPublisher" not in globals():
    ...
    theLogPublisher = LogPublisher(observerPublisher=newGlobalLogPublisher,
...
if "logfile" not in globals():
    logfile = LoggingFile(logger=(NewLogger()), level=(NewLogLevel.info),
      encoding=(getattr(sys.stdout, "encoding", None)))
    logerr = LoggingFile(logger=(NewLogger()), level=(NewLogLevel.error),
