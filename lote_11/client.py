...
# Embedded file name: twisted\web\client.py
"""
HTTP client.
"""
...
class HTTPPageGetter(http.HTTPClient):
        ...
        if not l:
            ...
            if self.followRedirect:
                ...
                if self.factory._redirectCount >= self.factory.redirectLimit:
                    err = error.InfiniteRedirection((self.status),
                ...
                self.handleStatusDefault()
                self.factory.noPage(Failure(error.PageRedirect((self.status),
                  (self.message), location=url)))
        self.quietLoss = True
        self.transport.loseConnection()
    ...
    def handleResponse(self, response):
        ...
        elif self.factory.method == b'HEAD':
            self.factory.page(b'')
...
class HTTPClientFactory(protocol.ClientFactory):
    ...
    def __init__(self, url, method=b'GET', postdata=None, headers=None, agent=b'Twisted PageGetter', timeout=0, cookies=None, followRedirect=True, redirectLimit=20, afterFoundGet=False):
        ...
        if postdata is not None:
            self.headers.setdefault(b'Content-Length', intToBytes(len(postdata)))
...
class HTTPDownloader(HTTPClientFactory):
    ...
    def __init__(self, url, fileOrName, method=b'GET', postdata=None, headers=None, agent=b'Twisted client', supportPartial=False, timeout=0, cookies=None, followRedirect=True, redirectLimit=20, afterFoundGet=False):
        ...
        HTTPClientFactory.__init__(self,
          url, method=method, postdata=postdata, headers=headers, agent=agent,
          timeout=timeout,
          cookies=cookies,
...
def _deprecateGetPageClasses():
    ...
    for klass in [
     ...
     HTTPClientFactory, HTTPDownloader]:
        deprecatedModuleAttribute(Version("Twisted", 16, 7, 0), getDeprecationWarningString(klass,
          (Version("Twisted", 16, 7, 0)),
          replacement=_GETPAGE_REPLACEMENT_TEXT).split("; ")[1], klass.__module__, klass.__name__)
...
def getPage(url, contextFactory=None, *args, **kwargs):
 ...
 url, 
 HTTPClientFactory, *args, contextFactory=contextFactory, **kwargs).deferred
...
def downloadPage(url, file, contextFactory=None, *args, **kwargs):
    ...
    return _makeGetterFactory(
 url, 
 factoryFactory, *args, contextFactory=contextFactory, **kwargs).deferred


from twisted.web.error import SchemeNotSupported
from twisted.web._newclient import HTTP11ClientProtocol, PotentialDataLoss, Request, RequestGenerationFailed, RequestNotSent, RequestTransmissionFailed, Response, ResponseDone, ResponseFailed, ResponseNeverReceived, _WrapperException
try:
...
class WebClientContextFactory(object):
    ...
    def _getCertificateOptions(self, hostname, port):
        ...
        """
        return CertificateOptions(method=(SSL.SSLv23_METHOD),
...
deprecatedModuleAttribute(Version("Twisted", 14, 0, 0), getDeprecationWarningString(WebClientContextFactory,
  (Version("Twisted", 14, 0, 0)),
  replacement=BrowserLikePolicyForHTTPS).split("; ")[1], WebClientContextFactory.__module__, WebClientContextFactory.__name__)
...
class _AgentBase(object):
    ...
    def _computeHostValue(self, scheme, host, port):
        ...
        if isIPv6Address(nativeString(host)):
            host = b'[' + host + b']'
        if (
         scheme, port) in ((b'http', 80), (b'https', 443)):
            return host
        else:
    ...
    def _requestWithEndpoint(self, key, endpoint, method, parsedURI, headers, bodyProducer, requestPath):
        ...
        if not isinstance(method, bytes):
            raise TypeError("method={!r} is {}, but must be bytes".format(method, type(method)))
        else:
        ...
        def cbConnected(proto):
            return proto.request(Request._construct(method, requestPath, headers, bodyProducer, persistent=(self._pool.persistent),
              parsedURI=parsedURI))
...
class Agent(_AgentBase):
    ...
    def __init__(self, reactor, contextFactory=BrowserLikePolicyForHTTPS(), connectTimeout=None, bindAddress=None, pool=None):
        ...
        if not IPolicyForHTTPS.providedBy(contextFactory):
            warnings.warn((repr(contextFactory) + " was passed as the HTTPS policy for an Agent, but it does not provide IPolicyForHTTPS.  Since Twisted 14.0, you must pass a provider of IPolicyForHTTPS."),
              stacklevel=2,
...
class _FakeUrllib2Request(object):
    ...
    def __init__(self, uri):
        ...
        if (
         _uri.scheme, _uri.port) not in ((b'http', 80), (b'https', 443)):
...
class RedirectAgent(object):
    ...
    def _handleRedirect(self, response, method, uri, headers, redirectCount):
        ...
        if redirectCount >= self._redirectLimit:
            err = error.InfiniteRedirection((response.code),
              b'Infinite redirection detected',
              location=uri)
...
def readBody(response):
    ...
    if protocol.transport is not None:
        if getAbort() is None:
            warnings.warn("Using readBody with a transport that does not have an abortConnection method",
              category=DeprecationWarning,
              stacklevel=2)
    return d
