...
import errno, os, signal, socket, stat, subprocess, sys, threading, warnings
        ...
        def create_unix_connection(self, protocol_factory, path, *, ssl=None, sock=None, server_hostname=None):
            if not server_hostname is None:
                ...
                if path is not None:
                    ...
                    if sock.family != socket.AF_UNIX or not base_events._is_stream_socket(sock.type):
                        raise ValueError("A UNIX Domain Stream Socket was expected, got {!r}".format(sock))
                    sock.setblocking(False)
