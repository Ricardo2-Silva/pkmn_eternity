...
# Embedded file name: twisted\internet\_sslverify.py
...
_tlsDisableFlags = {(TLSVersion.SSLv3): (SSL.OP_NO_SSLv3), 
 (TLSVersion.TLSv1_0): (SSL.OP_NO_TLSv1), 
 (TLSVersion.TLSv1_1): (SSL.OP_NO_TLSv1_1), 
 (TLSVersion.TLSv1_2): (SSL.OP_NO_TLSv1_2), 
 (TLSVersion.TLSv1_3): (getattr(SSL, "OP_NO_TLSv1_3", 0))}
...
def _selectVerifyImplementation():
    ...
    try:
        ...
        return (
         verify_hostname, verify_ip_address, VerificationError)
    except ImportError as e:
        warnings.warn_explicit(("You do not have a working installation of the service_identity module: '" + str(e) + "'.  Please install it from <https://pypi.python.org/pypi/service_identity> and make sure all of its dependencies are satisfied.  " + whatsWrong),
...
def optionsForClientTLS(hostname, trustRoot=None, clientCertificate=None, acceptableProtocols=None, **kw):
    ...
    if clientCertificate:
        extraCertificateOptions.update(privateKey=(clientCertificate.privateKey.original),
          certificate=(clientCertificate.original))
    certificateOptions = OpenSSLCertificateOptions(trustRoot=trustRoot, 
     acceptableProtocols=acceptableProtocols, **extraCertificateOptions)
    ...
    def __init__(self, privateKey=None, certificate=None, method=None, verify=False, caCerts=None, verifyDepth=9, requireCertificate=True, verifyOnce=True, enableSingleUseKeys=True, enableSessions=True, fixBrokenPeers=False, enableSessionTickets=False, extraCertChain=None, acceptableCiphers=None, dhParameters=None, trustRoot=None, acceptableProtocols=None, raiseMinimumTo=None, insecurelyLowerMinimumTo=None, lowerMaximumSecurityTo=None):
        ...
        if (privateKey is None) != (certificate is None):
            ...
            if method is None:
                ...
                if raiseMinimumTo:
                    if lowerMaximumSecurityTo:
                        if raiseMinimumTo > lowerMaximumSecurityTo:
            ...
            if insecurelyLowerMinimumTo is None:
                ...
                if lowerMaximumSecurityTo:
                    if insecurelyLowerMinimumTo > lowerMaximumSecurityTo:
            ...
            if lowerMaximumSecurityTo:
                ...
                warnings.warn("Passing method to twisted.internet.ssl.CertificateOptions was deprecated in Twisted 17.1.0. Please use a combination of insecurelyLowerMinimumTo, raiseMinimumTo, and lowerMaximumSecurityTo instead, as Twisted will correctly configure the method.",
                  DeprecationWarning,
                  stacklevel=3)
                self.method = method
            if verify:
            ...
            if extraCertChain is not None:
                ...
                self._ecChooser = _ChooseDiffieHellmanEllipticCurve((SSL.OPENSSL_VERSION_NUMBER),
            ...
            if acceptableProtocols is not None:
                if not protocolNegotiationMechanisms():
