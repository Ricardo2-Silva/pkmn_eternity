...
# Embedded file name: pyglet\media\codecs\ffmpeg.py
"""Use ffmpeg to decode audio and video media.
"""
import tempfile
from ctypes import c_int, c_uint16, c_int32, c_int64, c_uint32, c_uint64, c_uint8, c_uint, c_double, c_float, c_ubyte, c_size_t, c_char, c_char_p, c_void_p, addressof, byref, cast, POINTER, CFUNCTYPE, Structure, Union, create_string_buffer, memmove
...
def ffmpeg_get_frame_ts(stream):
    ...
    timestamp = avutil.av_rescale_q(ts, stream.time_base, AV_TIME_BASE_Q)
    return timestamp
...
class FFmpegSource(StreamingSource):
    ...
    def __init__(self, filename, file=None):
        ...
        for i in range(file_info.n_streams):
            ...
            if isinstance(info, StreamVideoInfo):
                if self._video_stream is None:
                    stream = ffmpeg_open_stream(self._file, i)
                    self.video_format = VideoFormat(width=(info.width),
                      height=(info.height))
            ...
            if isinstance(info, StreamAudioInfo) and info.sample_bits in (8, 16, 24) and self._audio_stream is None:
                stream = ffmpeg_open_stream(self._file, i)
                self.audio_format = AudioFormat(channels=(min(2, info.channels)),
    ...
    def __del__(self):
        if hasattr(self, "_tempfile"):
            ...
            if ffmpeg_free_packet is not None:
                ffmpeg_free_packet(self._packet)
            if self._video_stream:
                if swscale is not None:
            ...
            if self._file:
                if ffmpeg_close_file is not None:
        ...
        if _debug:
            ...
            if not self.audio_format:
                ...
                while len(self.audioq) > 1 and len(self.videoq) > 1:
                    ...
                    if audioq_is_first:
                        if not correct_audio_pos:
                            self._get_audio_packet()
                    if not correct_video_pos:
                        self.get_next_video_frame(skip_empty_frame=False)
                    else:
                        break
    ...
    def _fillq(self):
        ...
        self._fillq_scheduled = False
        while len(self.audioq) < self._max_len_audioq:
            if len(self.videoq) < self._max_len_videoq:
                if self._get_packet():
    ...
    def _check_low_level(self):
        ...
        if len(self.audioq) < 2 or len(self.videoq) < 2:
            if len(self.audioq) < self._max_len_audioq:
                if len(self.videoq) < self._max_len_videoq:
                    self._fillq()
            return True
        else:
    ...
    def _process_packet(self):
        ...
        if self._packet.contents.stream_index == self._video_stream_index:
            ...
            return video_packet
        if self.audio_format:
            if self._packet.contents.stream_index == self._audio_stream_index:
    ...
    def _ffmpeg_decode_audio(self, packet, data_out, compensation_time):
        ...
        if stream.type != AVMEDIA_TYPE_AUDIO:
            ...
            got_frame = c_int(0)
            bytes_used = avcodec.avcodec_decode_audio4(stream.codec_context, stream.frame, byref(got_frame), byref(packet))
            if bytes_used < 0:
                buf = create_string_buffer(128)
    ...
    def _ffmpeg_decode_video(self, packet, data_out):
        ...
        got_picture = c_int(0)
        bytes_used = avcodec.avcodec_decode_video2(stream.codec_context, stream.frame, byref(got_picture), byref(packet))
        if bytes_used < 0:
