...
# Embedded file name: twisted\internet\_resolver.py
"""
...
__metaclass__ = type
from socket import getaddrinfo, AF_INET, AF_INET6, AF_UNSPEC, SOCK_STREAM, SOCK_DGRAM, gaierror
from zope.interface import implementer
from twisted.internet.interfaces import IHostnameResolver, IHostResolution, IResolverSimple, IResolutionReceiver
...
_typesToAF = {(frozenset([IPv4Address])): AF_INET, 
 (frozenset([IPv6Address])): AF_INET6, 
 _any: AF_UNSPEC}
_afToType = {AF_INET: IPv4Address, 
 AF_INET6: IPv6Address}
_transportToSocket = {'TCP':SOCK_STREAM, 
 'UDP':SOCK_DGRAM}
_socktypeToType = {SOCK_STREAM: "TCP", 
 SOCK_DGRAM: "UDP"}
    ...
    def resolveHostName(self, resolutionReceiver, hostName, portNumber=0, addressTypes=None, transportSemantics='TCP'):
        ...
        def errorReceived(error):
            if not error.check(DNSLookupError):
                self._log.failure("while looking up {name} with {resolver}", error,
                  name=hostName, resolver=(self._simpleResolver))
