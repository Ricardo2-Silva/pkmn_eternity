...
# Embedded file name: pyglet\canvas\win32.py
...
class Win32Screen(Screen):
    ...
    def set_mode(self, mode):
        if not mode.screen is self:
            raise AssertionError
        else:
            if not self._initial_mode:
                self._initial_mode = self.get_mode()
            r = _user32.ChangeDisplaySettingsExW(self.get_device_name(), byref(mode._mode), None, CDS_FULLSCREEN, None)
