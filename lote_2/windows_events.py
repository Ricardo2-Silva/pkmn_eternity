    ...
    def _unregister_wait(self):
        ...
        try:
            ...
            if exc.winerror != _overlapped.ERROR_IO_PENDING:
                context = {'message':"Failed to unregister the wait handle",  'exception':exc, 
                 'future':self}
                if self._source_traceback:
                    context["source_traceback"] = self._source_traceback
                self._loop.call_exception_handler(context)
    ...
    def start_serving_pipe(self, protocol_factory, address):
        ...
        def loop_accept_pipe(f=None):
            ...
            try:
                ...
                if pipe:
                    if pipe.fileno() != -1:
                        self.call_exception_handler({'message':"Pipe accept failed", 
                         'exception':exc, 
                         'pipe':pipe})
                        pipe.close()
                if self._debug:
        ...
        while 1:
            ...
            try:
                ...
                if self._loop.get_debug():
                    self._loop.call_exception_handler({'message':"GetQueuedCompletionStatus() returned an unexpected event", 
