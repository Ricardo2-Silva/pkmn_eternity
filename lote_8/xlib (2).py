...
# Embedded file name: pyglet\gl\xlib.py
import warnings
from ctypes import *
...
class XlibConfig(Config):
    ...
    def match(self, canvas):
        ...
        try:
            return [
             config_class(canvas, info, attrib_list, self)]
        except gl.ContextException:
            return []


class BaseXlibCanvasConfig(CanvasConfig):
    attribute_ids = {'buffer_size':glx.GLX_BUFFER_SIZE, 
     'level':glx.GLX_LEVEL, 
...
class XlibCanvasConfig13(BaseXlibCanvasConfig):
    ...
    attribute_ids.update({'sample_buffers':glx.GLX_SAMPLE_BUFFERS, 
    ...
    def __init__(self, canvas, glx_info, fbconfig, config):
        ...
        for name, attr in self.attribute_ids.items():
            value = c_int()
            result = glx.glXGetFBConfigAttrib(x_display, self._fbconfig, attr, byref(value))
            if result >= 0:
                setattr(self, name, value.value)
...
class BaseXlibContext(Context):
    ...
    def set_vsync(self, vsync=True):
        ...
        try:
            if not self._use_video_sync:
                if self._have_EXT_swap_control:
            ...
            if self._have_SGI_swap_control:
    ...
    def _wait_vsync(self):
        if self._vsync:
            if self._have_SGI_video_sync:
                if self._use_video_sync:
                    count = c_uint()
...
class XlibContext13(BaseXlibContext):
    ...
    def _create_glx_context(self, share):
        if self.config.requires_gl_3():
            ...
            share_context = None
        return glx.glXCreateNewContext(self.config.canvas.display._display, self.config._fbconfig, glx.GLX_RGBA_TYPE, share_context, True)
...
class XlibContextARB(XlibContext13):
    ...
    def _create_glx_context(self, share):
        ...
        return glxext_arb.glXCreateContextAttribsARB(self.config.canvas.display._display, self.config._fbconfig, share_context, True, attribs)
