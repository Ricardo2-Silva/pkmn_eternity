...
# Embedded file name: twisted\python\deprecate.py
"""
...
def deprecatedProperty(version, replacement=None):
    ...
    def deprecationDecorator(function):
        if _PY3:
            ...
            warningString = _getDeprecationWarningString(fqdn, version, None, replacement)
            ...
            if n < len(positional):
                ...
                if param.kind == inspect.Parameter.KEYWORD_ONLY:
                    if name not in keyword:
                        ...
                        raise TypeError("'{}' parameter is invalid kind: {}".format(name, param.kind))
        ...
        def wrapped(*args, **kwargs):
            ...
            for this, that in argumentPairs:
                if this in arguments and that in arguments:
                    raise TypeError("The %r and %r arguments to %s are mutually exclusive." % (
